...
//   the four repo-wide GUARDS (`scripts/*.test.ts`), which read `packages/` and `apps/` whole —
...
//   for an `errors.ts` that has gone unreachable, and allergen-names-drift pins the 14 EU allergen
//   display names equal across the till and dashboard i18n copies;
