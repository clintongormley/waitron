...
| [`inoguerols/verifactu`](https://github.com/inoguerols/verifactu) | MIT | **Yes** | TypeScript; the broadest-surface reference (verified 2026-09-21 against its source at tag 1.5.3): huella chaining, XML build, a mutual-TLS SOAP submission client, the consulta query, response parsing, XAdES signing, a compliance linter, QR image, a CLI and an MCP server. Ours is ahead only on structured, typed response/effective-state parsing and on XML **request** parsing (which it lacks); its response/consulta parsing is looser. Actively maintained. Best reference. (Corrected 2026-09-21 — an earlier "chaining, QR, lint only" note understated it.) |
