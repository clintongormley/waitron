...
> path is unchanged: the tip still never reaches `computeHuella`. The "Why it matters" text below
> predates that move and its `sales.tip_amount` reference; the current picture is in
