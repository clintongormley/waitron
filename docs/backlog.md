...
3. **Resolve the greyed-out Split/Move buttons.** **Cheap half — wire move/join/merge/transfer into the
   ...
   invisible to PGlite) with a real-PG race test. **Both small followups now LANDED (#181):** (a) a
   sole-anchor `unjoinTable` with items now throws an honest `table.not_shared` (409) instead of the
   misleading `tab.not_open`; (b) `priceOrderLines` early-returns for empty lines, so `createOpenOrder`
   no longer does the full-catalogue read on a lineless open order (splitOffCheck / lineless openTab /
   unjoin). Table-service core (TS-1..TS-5) + these followups are DONE.
...
- **Till operator-UI defaults now match the product default (en-GB) — LANDED #170.** `formatMoney` followed a hardcoded `es-ES` at every operator-UI call site, so money rendered `12,50 €` even though the strings were already English at runtime; it now defaults to `currentLocale()`, and the `t.ts` module default + the allergen screen's operator `locale` property default to `FALLBACK_LOCALE`. Fiscal `invoiceLocale` defaults stay `es-ES` (receipt language, separate axis). **Follow-up: the dashboard has the identical `es-ES` module default (`apps/dashboard/src/i18n/t.ts:7`) + `#venueLocale`, left untouched to avoid colliding with the in-flight `feat/dashboard-sidebar-email-login` branch — fold the same flip into that branch, or a small standalone change; check whether the dashboard's money formatter has the same "doesn't follow the UI locale" bug.**
