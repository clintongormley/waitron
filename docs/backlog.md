...
- **KDS (sub-project 12)** — **multi-station routing** (per-station displays, route each line to its
  ...
  **KDS-3 expo and the always-on device identity remain to spec, now unblocked**; fiscal firewall proven
  in Task 10 — see *Recently shipped* → KDS-1). **KDS-1** = `kitchen_stations` config (one
  ...
  firewall proven in Task 8** — see *Recently shipped* → KDS-2. Spec + plan in `docs/superpowers/{specs,plans}/2026-08-17-kds-2*`. **KDS-3 DESIGNED + PLANNED
  ...
  **bump-course-ready** across stations, and **away** (`ticket_items.away_at` — plated & dispatched,
  ...
  `expo` device kind follows device-identity). Non-fiscal; **now UNBLOCKED** (KDS-1 + KDS-2 built) — the
  last KDS slice. Spec + plan in
...
spin-off) specced + planned too, now unblocked (KDS-1 built). **KDS-2** (courses + fire control) is **now
BUILT (2026-08-22)** and **KDS-3** (expo/pass) is **specced + planned** (2026-08-17), now unblocked
(KDS-1 + KDS-2 built). So **the KDS track's design is complete** (KDS-1/2/3 + device identity), and only
KDS-3 remains to build. The **kitchen-printers** requirement grew into
