...
- **KDS-3** Kitchen Display System — expo / pass display (sub-project 12; **SHIPPED 2026-08-22, PR #133**). Depends on KDS-1 (#129) + KDS-2 (#131) + FP-1 (#127). **Schema (migration 0059):** a bare `ALTER TYPE fire_control_mode ADD VALUE 'expo'` (the third fire-control mode) + `ticket_items.away_at timestamptz NULL` (the dispatch / "plated & left the pass" marker) — both additive: the new enum value is used only at **runtime** (never inside a migration statement, so the single-tx migrator can't hit `55P04`), and the nullable column rides 0055's table-level FORCE RLS + grants, so **no RLS/grant change**. **Server:** `listExpoQueue` — a cross-station read aggregating every open order (node-scoped on `cfg.nodeId`, `abandoned` + **fully-away** orders excluded via an EXISTS the deletion-proof pins) grouped by course (on `display_order`, joining `kitchen_stations` for cross-station station names, with fired/away roll-ups); `bumpCourseReady(orderId, courseId)` (the `advanceTicket` set-shape keyed on COURSE, reusing `advanceSet('ready')`, skips held items, no-op on empty) and `markCourseAway(orderId, courseId)` (`requireCourse` existence → `course.not_found`, stamps `away_at = now()` on `ready`-not-away items only, idempotent) — both **deletion-proven** (+ a real-PG node-symmetric/cross-tenant RLS test for `listExpoQueue`, folded in from Task 2); and the FP-1 floor `enRoute` count (`away_at NOT NULL AND served_at NULL`) added into `listTablesWithState`'s tab LATERAL with the existing `readyToServe`/`pendingToServe`/`status`/`zoneId` **byte-unchanged** (deletion-proven, no regression). **API:** `GET /api/expo/queue` + `POST /api/orders/:id/courses/:courseId/ready` (bumpCourseReady) + `POST …/away` (markCourseAway) — session-gated, mirroring the KDS-2 fire route, **no STATUS additions** (`course.not_found`/`ticket.*` already mapped). The `FireControl` union gained `'expo'` across its enumeration sites (server + dashboard + till types), and the management-api validator now **derives** from `fireControlMode.enumValues` (closing the one drift site typecheck can't protect). **UI:** a session-gated **till expo screen** (`till-expo-screen` — card-per-order, courses grouped null-first, per-course levers by state: **fire** a held course only under `fire_control='expo'`, **"Curso listo"** on a fired course, **"En camino"** on an all-`ready` course; per-item name·qty·station·state; reuses the KDS-1 station widget's rendering + tokens), registered in the till Screen union + nav; the dashboard fire-control toggle gained an **expo** option ("Expo"/"Pase"); and the FP-1 floor now renders a single most-advanced hint (**en camino > listos > por servir**). Spanish user-facing copy, English identifiers. **Fiscal safety (H2) — proven, not asserted:** KDS-3 writes **no fiscal-core code**; the pay/collect path is byte-unchanged. The Task-7 **grep of the fiscal boundary** (`record-sale.ts` + `backend.ts` + `registro-row.ts`, and all of `packages/verifactu`/`packages/fiscal-verifactu`/`packages/core`) returns **zero** KDS-3 tokens (`away_at`/`awayAt`/`expo`/`enRoute`/`bumpCourseReady`/`markCourseAway`/`listExpoQueue`, with `expo` **word-anchored** so the `export` substring can't masquerade as a hit — verified in both directions), with a **live differential control** on `apps/server/src/working-order.ts` (**30 matches**) and `apps/till/src/screens/till-expo-screen.ts` (**21 matches**) proving the same pattern discriminates. **No huella pinning test was added** (as KDS-2 also chose): `away_at` is one more column on `ticket_items`, and the existing `served-at-huella.test.ts` KDS-1 block already files a registro with `ticket_items` fully populated through the real pay path and gets an **identical huella** — a huella invariant to the whole table's presence/absence is a fortiori invariant to one more column of it — re-run green post-KDS-3 (3/3), so a dedicated KDS-3 case would be redundant (receipt in the Task-7 report). Guard sweep green: `inmutabilidad` (8 tests) confirms the `ALTER TYPE` + `away_at` leave `ticket_items` `relforcerowsecurity = true`; unfiltered db `test:coverage` (43 files / 566 pass / 2 skip, 100/98.73/100/100) + root Vitest guards (634 tests, incl. `english-only`/`errors-reachable`/`guarded-teardowns`/`ci-workflow`) + lint/typecheck/format all pass; **no pinned list changed** (KDS-3 adds no package; the migration manifest is set-level so 0059 doesn't touch `migratedSets`; no scope/mutation-count/`vocabulary-scope` file touched). New enum value `'expo'` on `fire_control_mode`; **no new error codes** (no `errors.ts` touched). **So the KDS track's display slices are all BUILT (KDS-1 core + KDS-2 courses + KDS-3 expo); the always-on device identity remains to build (unblocked) — the expo screen ships session-gated, and the `expo` device kind that runs it always-on is a future additive that follows device-identity.** Deferred follow-ups under *Debt* → KDS-3. [spec](superpowers/specs/2026-08-17-kds-3-expo-pass-design.md) · [plan](superpowers/plans/2026-08-17-kds-3.md).
- **KDS-2** Kitchen Display System — courses & fire control (sub-project 12; **SHIPPED 2026-08-22, PR #131**). Depends on KDS-1 (#129) + FP-1 (#127). **Schema (migrations 0057/0058):** `kitchen_courses` (tenant-scoped, FORCE RLS + tenant-isolation policy + SELECT/INSERT/UPDATE grants, `(tenant_id, location_id, name)` unique, **no DELETE** — a course deactivates via `active`), a product-default `products.course_id` + a per-line `working_order_lines.course_id` + `ticket_items.course_id`/`ticket_items.fired_at` (all bare nullable uuid, composite FKs hand-written in 0058 per the KDS-1 station idiom), and a `locations.fire_control` venue setting (a `fire_control_mode` pgEnum, `waiter` default / `kitchen`; `expo` reserved for KDS-3). **Server:** course config verbs (`till.configure`-gated, `course.name_taken`/`course.not_found`), a ring-time **course resolver** (product default + per-line override), and **hold-and-fire** — the earliest course auto-fires, later courses hold (`fired_at` NULL), `fireCourse(orderId, courseId)` fires a held course (idempotent), a held item can't advance (`ticket.item_held`), a null-course line fires immediately — plus fire-control read/set, all extending KDS-1's `fireLines`/`advanceTicketItem`/`listStationQueue`. **API:** management-api course-config + fire_control routes; till-api `POST …/courses/:id/fire` (session-gated) + `ticket.item_held` (409). **UI:** the station display groups the queue by course + greys held items + shows the kitchen "Empezar curso" fire under `fire_control='kitchen'`; the tab-ordering screen gains a per-line course picker + the waiter "Fire <course>" action under `'waiter'`; the dashboard gains a **Cursos** panel (course CRUD + order) + the fire_control toggle + a product course field. Spanish user-facing copy, English identifiers. **Fiscal safety (H2) — proven, not asserted:** KDS-2 writes **no fiscal-core code**; the pay/collect path is byte-unchanged. The Task-8 **grep of the fiscal boundary** (`record-sale.ts` + `backend.ts` + `registro-row.ts`) returns **zero** KDS-2 tokens (`course`/`course_id`/`fired_at`/`fire_control`/`kitchen_courses`), with a **live differential control** on `apps/server/src/kitchen.ts` — the same pattern, **46 matching lines** — proving the grep discriminates. **No huella pinning test was added** (unlike KDS-1's `served_at`): the KDS-2 fields demonstrably cannot reach `computeHuella` — they live on the kitchen tables (`ticket_items`/`working_order_lines`/`products`/`locations`), `record-sale.ts` touches only `sales`/`sale_lines`/`tenders`/`registros_facturacion`, and the KDS-2 vocabulary is **absent from all of** `packages/verifactu`/`packages/fiscal-verifactu`/`packages/core` (grep exit 1); the existing `served-at-huella.test.ts` KDS-1 block already files a registro with **`fired_at`-bearing `ticket_items` present** through the real pay path and gets an **identical huella** — re-run green post-KDS-2 (3/3) — so a dedicated KDS-2 case would be redundant (receipt in the Task-8 report). Guard sweep green: `inmutabilidad` (8 tests) auto-covers `kitchen_courses` `relforcerowsecurity = true`; unfiltered db `test:coverage` (43 files / 564 pass, 100/98.74/100/100) + root Vitest guards (629 tests, incl. `english-only`/`errors-reachable`/`guarded-teardowns`/`ci-workflow`) + lint/typecheck/format all pass; **no pinned list changed** (KDS-2 adds no package; the migration manifest is set-level so 0057/0058 don't touch `migratedSets`; no scope/mutation-count/`vocabulary-scope` file touched). New codes `course.name_taken` / `course.not_found` / `ticket.item_held`. **KDS-3 expo/pass (adds the `expo` `fire_control` value + the pass display) is the last KDS slice, now unblocked.** Deferred follow-ups under *Debt* → KDS-2 (a small A1/A2/E2+E3 fast-follow: validate the courseId override, `fireCourse` existence-not-liveness, and merge the fireLines query pair). [spec](superpowers/specs/2026-08-17-kds-2-courses-fire-control-design.md) · [plan](superpowers/plans/2026-08-17-kds-2.md).
- **KDS-1** Kitchen Display System — stations, routing & the per-line ticket rework (sub-project 12; **SHIPPED 2026-08-22, PR #129**). Depends on TS-1 (#97) + FP-1 (#127). **Schema (migrations 0054/0055):** `kitchen_stations` (tenant-scoped, FORCE RLS + tenant-isolation policy + grants, one `WHERE is_default` partial-unique default, seeded 'Cocina' per venue) and **`ticket_items`** (per-line/per-station, FORCE RLS, `queued→preparing→ready` on a new `ticket_state` enum, `(tenant_id, working_order_line_id)` unique) — the latter **replaces `order_prep`** (dropped, pre-production); plus `categories.station_id`/`products.station_id` routing, `working_orders.collected_at` (order-level handover marker), and a `bump_mode` venue setting. **Server:** station config + routing verbs (`till.configure`-gated), a shared `fireLines` resolver (product override > category > default, station snapshotted at fire) reworking `placeOrder`/`sendToPrep`/`addTabRound`, `advanceTicketItem`/`advanceTicket` bumps, `listStationQueue` (grouped by order, node-scoped, abandoned+collected excluded), the counter collect ported to order-level `collected_at`, and the **ready→floor** read (`readyToServe` = ready-not-served → FP-1's "N listos"). **UI:** session-gated till station-display (kanban ⇄ rail, per-line/whole-ticket bump) + dashboard Cocina config. Spanish user-facing copy, English identifiers. **Fiscal safety (H2) — proven, not asserted:** KDS-1 writes **no fiscal-core code**; the pay/collect path is byte-unchanged. Task-10 **grep of the fiscal boundary** (`record-sale.ts` + `backend.ts` + `registro-row.ts`) returns **zero** KDS tokens (`station_id`/`ticket_items`/`collected_at`/`bump_mode`, with a live differential control on `working-order.ts`/`till-sale.ts`), and `served-at-huella.test.ts` gained a **KDS case**: an order run through the full kitchen lifecycle (fired → routed → advanced to `ready` → `collected_at` set, `ticket_items` present) and one filed plain file the **identical huella** under a frozen clock on two chains sharing one emisor NIF, with a self-check they genuinely differ in KDS state and a **negative control** (the huella comparison FAILS when the baskets differ, proving it discriminates — watched red then restored). Guard sweep green: `inmutabilidad` shows `kitchen_stations` + `ticket_items` `relforcerowsecurity = true`; unfiltered db `test:coverage` (100/98.73/100/100) + root Vitest guards (627 tests, incl. `english-only`/`errors-reachable`/`guarded-teardowns`) + lint/typecheck/format all pass; **no pinned list changed** (KDS-1 adds no package; the migration manifest is set-level so 0054/0055 don't touch `migratedSets`; the retired `order_prep.invalid_transition` code stays declared per never-delete-shipped-codes). **KDS-2 courses, KDS-3 expo, and the always-on device identity remain to spec (now unblocked).** Deferred follow-ups under *Debt* → KDS-1. [spec](superpowers/specs/2026-08-17-kds-1-stations-routing-tickets-design.md) · [plan](superpowers/plans/2026-08-17-kds-1.md).
- **#127** Floor plan **FP-1** — the operable live floor (sub-project 11, slice 1; TS-1/TS-2's UI): tables become occupancy-aware surfaces. **Schema (migrations 0051/0052):** `floor_zones` (tenant-scoped, FORCE RLS + tenant-isolation policy + grants), `dining_tables.zone_id` (composite FK, replacing the free-text `zone`), `working_order_lines.served_at` (nullable, pre-fiscal). **Server:** zone/table config verbs, `markLineServed`/`unmarkLineServed`, `listTablesWithState.pendingToServe`, `readTabLines` (locked prices), a session-gated status catalogue read. **API:** management-API zone/table config routes (`till.configure`-gated) + till-API floor routes. **UI:** till **live-floor screen** (occupancy-coloured cards grouped by zone) + **table-ordering screen** (round bar + pull-out served/pay drawer) + dashboard **floor-config** screen. Cards, **not a spatial map** (FP-2 = the canvas). Spanish user-facing copy, English identifiers. **Fiscal safety (H2) — proven, not asserted:** paying a tab **reuses `recordSale`** (no new fiscal code, files the tab's STORED locked lines, no re-price); `served_at` **cannot reach a huella** — structural (zero-hit in `record-sale.ts`/`backend.ts`; the pay path's `readLockedLines` never selects it) **and** behavioural (`served-at-huella.test.ts`: two identical baskets, one fully served / one unserved, file an **identical huella** under a frozen clock on two chains sharing one emisor NIF, with a self-check they genuinely differ in `served_at`); a finish-branch **mutation probe** confirmed the append-only trigger rejects a served write on a settled order. No fiscal table touched; `inmutabilidad` sees `floor_zones` FORCE RLS. Subagent-driven TDD (11 tasks, Task 9 split 9a/9b) → per-task reviews + fix loops → whole-branch review (2 stale-comment nits) → finish-branch: **simplify** extracted a shared `grossOf` money primitive (one rounding path across basket/receipt/tab), a `withVenueAuth` gate helper, and de-duplicated `requireLineNo`; the **two reviewers RAN** the pay path, the concurrency suites (63 tests, no new lock inversion), and the RLS/gate suites — **0 correctness bugs**. **Owner caught Spanish IN IDENTIFIERS** (screen keys, i18n key namespaces, CSS classes) that the `english-only` guard doesn't scan in `apps/*` — swept `sala`→`floor`, `por-servir`→`to-serve` (Spanish i18n *values* kept). **Two CI-only failures, both root-caused not silenced:** (1) FP-1's `floor-zones.ts` grew `@waitron/db` 43→44 mutate-slices, tripping a **hard-coded `44` in main's `mutation-shard` test** (the §1 "count goes stale when scope changes" class) — fixed to derive the count from the live slice total; (2) the added "Sala" button **overflowed the non-wrapping counter header**, pushing `.operator`'s centre off-viewport on CI's wider Linux fonts, where axe fell back to a white test canvas the real app never shows (`index.html` themes `body`) — fixed by painting the themed canvas in the a11y harness (app is correct; test-infra only, all 3 copies). **Copilot:** one real twin-miss — the table POST/PATCH routes screened `:id` but not `body.zoneId` (malformed → `22P02`→500); fixed in `till-api` **and** `management-api` (same gap), 3 threads resolved. 893 server / 515 till / 644 dashboard tests; all shards green. Deferred follow-ups under *Debt* → floor plan FP-1; **FP-2** (spatial canvas + on-till/dashboard editor, tap-to-place) **BUILT 2026-08-21** — see the sub-project 11 row. [spec](superpowers/specs/2026-08-17-floor-plan-fp1-live-floor-design.md) · [plan](superpowers/plans/2026-08-17-floor-plan-fp1.md).
...
- **Floor plan (sub-project 11)** — a layout editor **with live occupancy** (table state tied to
  ...
  map**. **FP-2 DESIGNED + PLANNED 2026-08-17, BUILT 2026-08-21** adds the **spatial canvas + edit mode** (designed drag-to-place, **built tap-to-place** — see below) —
  ...
  **frozen clock**, as first records sharing one emisor NIF (two tenants → no `registros_identidad_uq`
  ...
  TAP-TO-PLACE** — a deliberate deviation from spec §5a's "drag-to-place" wording (**owner decision**):
  tap an unplaced tray table in edit mode → default slot → reposition via the canvas's existing
  drag/keyboard. **On-till editing is MANAGER-ON-TILL** (the operator's own `till.configure` role, the
  first till route to call `authorize()`, which now surfaces the session role so the till can gate the
  toggle while the server re-checks; **supervisor-PIN override deferred**). A **Task-7b follow-up**
  (`de88a14`: `listTables` now projects `posX/posY/shape/rotation`, mirroring `listTablesWithState`)
  made the dashboard editor functional end-to-end — before it, a placed table snapped back to the tray
  on reload. **The fiscal firewall is proven, not asserted (H2/§4):** the placement columns live on
  `dining_tables`, structurally even further from the huella than `served_at` (a `working_order_lines`
  field); a **zero-hit fiscal-boundary grep** over `record-sale.ts`/`backend.ts`/`registro-row.ts`
  (structural — only the English word "shape" in doc comments matches, confirmed with a differential
  control that the grep is live) **and** an extended `served-at-huella.test.ts` filing an identical
  basket for a **placed** table vs a **walk-up** under a frozen clock → **identical huella**, with a
  self-check they genuinely differ in placement (behavioural, proven by a negative control that the
  self-check fails when both are placed alike). Full guard sweep green — `inmutabilidad` shows
  `dining_tables` relforcerowsecurity = true (migration **0053** is an `ALTER … ADD COLUMN` + the
  `floor_table_shape` enum, **not a CREATE**, so FORCE RLS is unchanged), db/server/root `test:coverage`
  + the tree-wide guards + lint/typecheck/format all pass; no pinned list (manifest / `GENERIC_PACKAGES`
  / `vocabulary-scope` / migration count) changed. **The floor-plan surface is now complete through
  FP-2**; **KDS + bookings remain to spec.** Deferred follow-ups under *Debt* → floor plan FP-2.
- **KDS (sub-project 12)** — **multi-station routing** (per-station displays, route each line to its
  ...
  **KDS-3 expo BUILT 2026-08-22**; the always-on device identity **BUILT 2026-08-25 (PR #134)**; fiscal
  ...
  `served_at`). **KDS-2 BUILT 2026-08-22** (designed + planned 2026-08-17) — venue-configured `kitchen_courses`, a
  ...
  `course_id` on `ticket_items`; held items can't advance (`ticket.item_held`). Non-fiscal; **fiscal
  firewall proven in Task 8** — see *Recently shipped* → KDS-2. Spec + plan in `docs/superpowers/{specs,plans}/2026-08-17-kds-2*`. **KDS-3 BUILT
  2026-08-22** — a **dedicated expo/pass display** aggregating every open order across all stations by
  ...
  **bump-course-ready** across stations, and **away** (`ticket_items.away_at` — plated & dispatched,
  feeding the floor an "en camino" hint between kitchen-`ready` and waiter-`served`). The expo screen
  ships **session-gated**; an `expo` device kind (once device-identity lands) is a future additive that
  ...
  identity-1 is also BUILT (2026-08-25, PR #134 — see its own row below)**. **Kitchen printers (owner requirement
  ...
  **always-on device identity-1** is **BUILT 2026-08-25, PR #134** (see its own row below).
...
- **Device identity-1 (KDS spin-off, sub-project 12)** — **BUILT 2026-08-25, PR #134** (was DESIGNED +
...
(2026-08-25, PR #134)** — enrol + device cookie + `requireDevice` + the Devices screen + the spec §8
...
- **Floor plan FP-1 follow-ups (sub-project 11). None blocking; surfaced during the build + the fiscal-firewall/guards task.**
  ...
  - **`parseCapacity` (management-api) and `requireCapacity` (till-api) duplicate the int4 capacity check.** Same
    ...
    caller appears.
- **Floor plan FP-2 follow-ups (sub-project 11). None blocking; surfaced during the FP-2 build + the fiscal-firewall/guards task.**
  - **Supervisor-PIN override for on-till "Editar plano" is deferred.** On-till floor editing is
    **manager-on-till** — `authorize(till.configure)` against the operator's own role (manager/admin hold
    it, supervisor/staff don't), the first till route to call `authorize()`. A supervisor-PIN override
    (let a supervisor unlock editing on a staff till) is deferred to the reusable
    `authorize()`-with-supervisor-override dialog the **`cash.drawer` authorization** slice introduces.
    No new permission was minted (the #81 venue-config `till.configure` role is reused).
  - **A zoneless ("Sin zona") table can't be tap-placed on-till.** `setTablePlacement` requires a LIVE
    zone, so tapping an unplaced table that belongs to no zone is refused server-side; the client swallows
    + refreshes (tested). A place-into-zone affordance (or a "pick a zone" step) is the follow-up; the
    workaround is to assign the table a zone first. Pairs with FP-1's "no way to clear a table's zone".
  - **Two tray taps in the same edit cycle can seed the same default slot.** Tap-to-place picks a default
    coordinate, so two quick taps before a reload can overlap (cosmetic — a drag resolves it). A
    next-free-slot search is the follow-up.
  - **`wt-table-token` is a near-duplicate of the FP-1 till token.** T5 extracted the FP-1 occupancy token
...
- **KDS-1 follow-ups (sub-project 12). None blocking; surfaced during the build, the reviews, and the
  fiscal-firewall/guards task.**
  - **Dashboard Cocina config is write-only — the editors can't reflect the PERSISTED value.** The
    per-category / per-product station routing `<select>`s and the `bump_mode` toggle send a write on an
    explicit change but cannot show the current setting: T7's catalogue reads don't project
    `categories.station_id` / `products.station_id`, and there is no management GET for `bump_mode`.
    Confirmed **display-only, no clobber** (writes fire only on an explicit change, so an unread field is
    never sent back). Follow-up: project station routing on the catalogue read and add a management
    `bump_mode` read so the editors render current state.
  - **`listStationQueue` shows ALL states until the order collects (spec §3c wanted `state != 'ready'` +
    a short ready-tail).** The plan/impl keeps a `ready` line on the station display until its order
    collects, rather than dropping it with a brief ready-tail. Defensible (a cook sees done-but-not-handed-off
    work), but **tab orders never collect** (`collected_at` is a counter-handover marker), so a tab's `ready`
    tickets linger on the queue indefinitely. Follow-up: clear a ready ticket when its line is served, or
    implement the spec's short ready-tail.
  - **Age colouring is rail-view only.** The kanban cells carry no age accent (only the ticket-rail cards
    do). Reasonable given the kanban layout; add a cell age accent if wanted.
  - **All-lines-cancelled delivery order stops counting as pending-delivery (untested edge).** With
    `ticket_items` CASCADE-deleted per line (via the `working_order_lines` FK), a delivery order whose every
    line is cancelled has no ticket items left, so it no longer counts as pending-delivery — where the old
    order-level `order_prep` row would have kept it counted. Arguably an improvement (a fully-cancelled order
    isn't pending anything), but there is no test pinning the edge either way.
  - **Migration 0055 has no paired snapshot (hand-written journal entry).** The custom RLS migration
    `0055_kds1_stations_tickets_rls.sql` was added to the drizzle journal by hand with no `meta/0055_snapshot.json`.
...
- **KDS-2 follow-ups (sub-project 12). None blocking.** The **A1/A2/E2+E3 fast-follow SHIPPED 2026-08-22, PR #132**: the per-line `courseId` override is now `isUuid`+`requireLiveCourse`-screened
  ...
  - **`advanceTicketItem`'s error disambiguation is a post-UPDATE read (E4 refactor, non-issue).** The held
    ...
    losing caller sees — benign (both refuse the same action, both 4xx, the pre-refactor read-first form had
    the same TOCTOU window). Noted, no action.
  - **Two owner-facing UX choices, both spec-consistent, trivially changed if wanted.** (a) The station
    display defaults to the kanban lens and the "Empezar curso" kitchen-fire button is rail-only, so a
    `fire_control='kitchen'` operator must toggle to Comandas to fire (mirrors the collect UX). (b) The tab
    course picker offers the venue's courses with no explicit "no course / fire now" option (keeps the
    `override ?? product.course_id` resolver; adding it would need the undefined-vs-null distinction). Both
    are owner decisions, not defects.
- **KDS-3 follow-ups (sub-project 12). None blocking; surfaced during the build and the task reviews.**
  - **Courseless (null-course) orders get no expo lever and no "en camino" pass step.** The expo screen's
    ready/away levers and the `bumpCourseReady`/`markCourseAway` routes are keyed on a `courseId`, so an
    order whose lines carry no course (`course_id` NULL — a null-course line fires immediately per KDS-2)
    has nothing for the pass to bump-ready or dispatch: those items advance on the KDS-1 station display and
    never take an `away_at` / "en camino" hint. Matches the routes as landed and the station precedent, and
  ...
  - **Owner-facing UX: the FP-1 floor now shows ONE most-advanced hint, not KDS-1's two badges.** Per spec
    ...
    two badges co-appearing, so no assertion was lost; a quick revert restores both counts if the owner
    prefers them shown side by side.
