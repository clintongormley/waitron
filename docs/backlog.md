...
5. *The demo path demands real business details nobody will use.* Reaching a demo box means filling
   ...
   asking for everything. **The tax ID is generated** (owner, 2026-09-12): the check letter is just
   the eight digits modulo 23 indexed into `TRWAGMYFPDXBNJZSQVHLCKE`
   (`packages/country-es/src/spain.ts`), so the demo path picks a number and computes its letter
   rather than asking for one. Ran the owner's example through the real validator: `61747065-T`
   comes back valid, normalized to `61747065T`, kind `personal`; the same number with any other
   letter is rejected on the checksum. Two notes for whoever builds it. That kind is `personal` — a
   DNI belongs to a person, while a restaurant would normally hold an entity NIF (a letter, seven
   digits and a control character, also generated by a checksum in the same file); nothing in the
   codebase reads the kind today, so either passes, but an entity one reads as a business. And a
   generated number is safe here only because a demo box files nothing to AEAT — the same generator
   must never be reachable from Prepare or Live. The remaining design question is where the made-up
   values are applied: the screen skipping fields, or the shell patching defaults into the draft, and
   how the review step shows what was chosen.
