...
- **KDS (sub-project 12)** — **multi-station routing** (per-station displays, route each line to its
  ...
  **KDS-1 BUILT 2026-08-22** (designed + planned 2026-08-17 — brainstormed with the owner via the visual
  companion, full scope chosen then sliced **KDS-1 → KDS-2 → KDS-3**; **KDS-2 courses, KDS-3 expo, and the
  always-on device identity remain to spec, now unblocked**; fiscal firewall proven in Task 10 — see
  *Recently shipped* → KDS-1). **KDS-1** = `kitchen_stations` config (one
