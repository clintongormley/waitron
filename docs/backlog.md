...
| **16 — Workforce** | *Registro de jornada* legal floor **DONE (#47)**. Remaining: **D2 scheduling** (shifts/rosters + `convenio_config`; the overtime *rule* the both-model projection computes is convenio-driven — an **asesor-laboral** call, not code) and **D3 payroll export** (integrate-not-build). Deferred edges: the registro export doesn't yet surface overtime (belongs to the payslip/D3); the correction period-fetch is a ±1-day window (a >1-day-relocation correction is out of the floor's scope, chained but maybe missed by the period fetch). A post-#47 `/finish-branch` review (follow-up PR, in flight) corrected four floor defects: the registro export rendered UTC instead of local wall-clock; the tamper chain omitted a correction's reason/actor and the capturing till; correction precedence tie-broke on the unhashed `ingest_seq` (a floor-bypasser could reorder corrections undetected) — now on the hashed `sequence_no`; and a `clockIn`/`clockOut` TOCTOU (an unlocked state read before the chain-head lock let two concurrent same-person clock-ins append a double-`in` that undercounts worked time) — now serialized per person with a `persons` row lock proven by a real-PG concurrency test |
