...
5. *The demo path demands real business details nobody will use.* Reaching a demo box means filling
   ...
   asking for everything. **The tax ID is generated, and it is a company one** (owner, 2026-09-12).
   Both shapes are a checksum over the digits (`packages/country-es/src/spain.ts`), so the demo path
   picks a number and computes the control character rather than asking anyone for one. Ran both of
   the owner's examples through the real validator. A company number — `B-4943574-6` — comes back
   valid, normalized to `B49435746`, kind `entity`, and the same digits with any other control
   character are rejected on the checksum; that is the shape a restaurant actually holds, so use it.
   A personal DNI also passes (`61747065-T`, kind `personal`, the letter being the eight digits
   modulo 23 indexed into `TRWAGMYFPDXBNJZSQVHLCKE`), but it belongs to a person rather than a
   business. Nothing in the codebase reads the kind today, so either would get through; the company
   one simply reads correctly on an invoice. The hyphens are fine either way — the validator strips
   spaces and hyphens before checking. One warning: a generated number is safe only because a demo
   box files nothing to AEAT, so the generator must never be reachable from Prepare or Live. The
