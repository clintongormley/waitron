...
- **Counter POS follow-ups (sub-project 7, slice 2 / 7b — park & retrieve, merged #61). None blocking;
  ...
  - **Park itself is not idempotent.** A re-sent `POST /api/working-orders` with a VALID id PK-collides
    (`23505`) → opaque 500 (the first park succeeded and the order IS in the held list). No fiscal effect;
    could catch the `23505` and replay the existing order like `payWorkingOrder` does. The route comment
    now states this accurately (Copilot, #61). (The distinct MALFORMED-park-id case — a non-uuid `body.id`
    `22P02`ing at the INSERT — is now screened to a 400 by #99; this remaining bullet is only the valid-id
    23505 replay.)
