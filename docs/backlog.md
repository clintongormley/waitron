...
- **Table service (foundation, sub-project 10 — *tabs*)** — the *tables* primitive the other three
  ...
  **TS-4** transfer items (whole/partial, never re-priced) → **TS-5** split-bill (item-split — the one
  ...
  merge (see the *Recently shipped* row); **TS-4 (transfer items) is now the next buildable slice** (TS-5, FP-1, KDS-1,
  bookings-integration all unblock behind the chain). **#97 follow-ups:**
...
tabs), configurable service statuses, and move / join / merge are built; **TS-3 LANDED (#119, 2026-08-20)** so
**TS-4 (transfer items) is the next buildable slice**,
...
- **CI test-tier speedup — remaining rollout (from #112 phase 1 + #113 phase 1b + #114 phase 2 + #116 phase 3 + #117 phase 4 + #118 phase 5 + #120 phase 6 + #121 phase 7 + #122 phase 8).** The
  shared-container / template-DB harness is landed and proven across **all** of apps/server (#112 = the
  14 manifest/`asAppUser`-only suites; #113 = the 9 that needed role/grant/multi-template handling),
  **`@waitron/db`** (#114 — the `describeEachTarget` seam + the 12 `useRealPostgres` suites),
  **`@waitron/payments`** (#116 — 8 suites, `core_payments`, 3 probe roles, kept `singleFork`),
  **`@waitron/fiscal-verifactu`** (#117 — 11 suites, `core_fiscal`, 4 roles incl. a shared `rls_probe`, `maxForks: 4`) AND
  **`@waitron/workforce`** (#118 — 7 suites, `core_identity_workforce`, 3 roles, kept `singleFork`). What
  remains is the other real-PG packages. Plan + per-phase steps:
  `docs/superpowers/plans/2026-08-19-shared-test-container.md`.
  - **Reusable primitives now in place (from #113/#114/#116), don't reinvent per package:**
  ...
  - **P9..Pn — the remaining 7 real-PG packages** (scheduler,
    recipes, catalogue, credentials, purchasing, layouts, workforce-es) — all 1-2 files each; the tail
    of the rollout, to finish in one or two more batches. Smaller ones can BATCH into one
    PR (user OK'd 2026-08-20). Packages whose RLS suites use `asAppUser` (SET ROLE) rather than a probe
    LOGIN role need NO cluster roles in their globalSetup (e.g. reporting, core). Full-manifest packages
    use the `manifest` template key (bare `runMigrationSets(migrationOptionsFor(manifestSets(), null))`), the
    `manifest` template key (bare `runMigrationSets(migrationOptionsFor(manifestSets(), null))`), the
    `core_<schema>` convention otherwise. Each phase: add the package's globalSetup with its template
    set(s) + roles,
...
- **Table service TS-3 follow-ups (#119). None blocking; surfaced by the finish-branch reviews + Copilot.**
  ...
  - **`moveTabLines` source-not-open branch has no direct test.** The `from` open-check throw is unexercised (the
    ...
    `require_open_parent` trigger backstops it (worst case a 500, not data loss). Natural to add a symmetric test in
    **TS-4**, where `moveTabLines` first gains a direct external caller with a possibly-not-open source.
