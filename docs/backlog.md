...
- **CI test-tier speedup — remaining rollout (from #112 phase 1 + #113 phase 1b + #114 phase 2 + #116 phase 3).** The
  ...
  **`@waitron/db`** (#114 — the `describeEachTarget` seam + the 12 `useRealPostgres` suites) AND
  **`@waitron/payments`** (#116 — 8 suites, one `core_payments` template, 3 probe roles, kept `singleFork`). What
  ...
  - **Fork mode is a per-package call, not a default.** Two distinct reasons force fewer forks, and
    ...
    rather than serializing; `packages/payments` (#116) is the reason-(a) reference — it kept
    `singleFork` for the coverage-v8 artifact, and under one fork the connection budget is a non-issue
    (one file at a time), so no `maxForks`. Pick per package; document which reason applies.
  - **P4..Pn — the remaining ~13 real-PG packages** (fiscal-verifactu, workforce, sync,
