...
- **Table service (foundation, sub-project 10 — *tabs*)** — the *tables* primitive the other three
  ...
  **TS-4** transfer items (whole/partial, never re-priced) → **TS-5** split-bill (item-split — the one
  ...
  configurable service statuses (see the *Recently shipped* row); **TS-3 (move / join / merge) is now the next
  buildable slice** (TS-4→TS-5, FP-1, KDS-1, bookings-integration all unblock behind the chain). **#97 follow-ups:**
...
tabs) and configurable service statuses are built; **TS-3 (move / join / merge) is the next buildable slice**,
...
- **CI test-tier speedup — remaining rollout (from #112, phase 1).** The shared-container / template-DB
  harness is landed and proven on apps/server; the win compounds across the rest of the real-PG tier.
  Plan + per-phase steps: `docs/superpowers/plans/2026-08-19-shared-test-container.md`.
  - **Phase 1b — the 10 apps/server suites still on per-file containers.** `boot` (per-DB grants + a
    spawned server), the sync `setup`+grant suites (`sync-api`, `sync-e2e`), the `probeRole` suites
    (`pass`, `till-api`, `till-sale-integrated`, `webhook`), the CORE / CORE+IDENTITY-template suites
    (`clear-table-status`, `service-statuses`), and `dev-setup` (bare — the code under test IS the
    migrator). Each needs the shared model to grow role/grant/multi-template handling; `dev-setup` and
    the raw-container primitive tests (`db` `client`/`migrate`/`postgres`, `migrations` `apply.concurrency`,
    the 3 `provisioning` suites) stay per-container by design.
  - **P2..Pn — the other 15 real-PG packages.** Convert `useRealPostgres` → `useTemplateDb` per package,
    ...
    distinct-backend), measure the shard's CI drop.
...
- **CI Playwright apt slowness — the `--with-deps` step is the browser-shard long pole, slow and
  high-variance.** All three browser shards (`test-ui`, `test-till`, `test-dashboard`,
  `.github/workflows/ci.yml`) restore a `~/.cache/ms-playwright` browser-binary cache (reliably 2–8s,
  hits every run) then **unconditionally** run `pnpm --filter @waitron/<pkg> exec playwright install
  --with-deps chromium`, whose `--with-deps` half shells out to `apt-get` for Chromium's OS libraries
  on **every** run — the cache does not cover apt. Measured across the last 15 CI runs (each step's
  `startedAt`/`completedAt` from the Actions API): that step ranges **12s → 601s (~10 min)** with no
  ceiling (601s = run 32277880914 · test-till, job total 647s; 398s = run 32210239676 · test-till),
  while the actual shard is ~30–40s — so the apt step, not pnpm and not the browser download, is the
  cost. It **hung twice landing #110** (each time cancelled + re-run; passed on attempt 3), which is
  what surfaced this. Fix candidates — each **proven by a real passing CI run** (§1, not asserted; and
  measure the failing direction, sampling several runs since apt is high-variance): drop to plain
  `playwright install chromium` if `ubuntu-latest` already ships Chromium's libs (prove a missing lib
  actually breaks the suite, not silently passes); run `--with-deps` only on cache miss; `--only-shell
  chromium` if headed mode is never needed; or cache/pre-bake the apt layer. Fix all three shards
  identically (shared pattern) and update the explanatory comments above each step (stale comments are
  §1's #1 defect class). Full ceremony (feature branch, gate, PR, CI + Copilot). **Non-blocking** — a
  re-run clears it.
...
- **Table service TS-2 follow-ups (#103). None blocking; surfaced by the finish-branch reviews + Copilot.**
  ...
  - **TS-3 cross-slice caveat.** The reset trigger clears `WHERE tab_id = NEW.id` (future-proofed for a tab
    ...
    semantics the trigger and `listTablesWithState` rely on.
