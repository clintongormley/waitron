...
- **Upgrade testing — blocking before go-live (owner, 2026-09-26).** On 2026-09-26 the box could not
  ...
  could see because every suite migrates a fresh database. `scripts/migration-upgrade.test.ts` now
  ...
  - **A rebuild of a table another set's trigger names is still refused**, on a box that has the
    trigger: core `0003_variant_inherited_nullable` rebuilding `products` under media's triggers
    fails its closing rename. The guard steps over it by applying everything up to `0003` in one
    go, so the next such rebuild fails the guard, but nothing yet makes one possible. Decide the
    fix (for example, the migration drops and recreates the other set's triggers itself).
