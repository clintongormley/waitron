...
| **16 — Workforce** | *Registro de jornada* legal floor **DONE (#47)**. Remaining: **D2 scheduling** (shifts/rosters + `convenio_config`; the overtime *rule* the both-model projection computes is convenio-driven — an **asesor-laboral** call, not code) and **D3 payroll export** (integrate-not-build). Deferred edges: the registro export doesn't yet surface overtime (belongs to the payslip/D3); the correction period-fetch is a ±1-day window (a >1-day-relocation correction is out of the floor's scope, chained but maybe missed by the period fetch); the `clockIn`/`clockOut` state read is an unlocked SELECT taken before the chain-head lock, so two concurrent same-person clock-ins can both read "out" and append a double-`in` that the projection resolves by keeping the second — undercounting worked time. A real TOCTOU, **deferred to D2**: the clock state machine has no production caller until the till path lands, a fix needs per-person serialization + a real-PG concurrency test, and that belongs with the caller D2 owns (fixing it in the #47 follow-up would collide with `feat/workforce-d2-scheduling`). Surfaced by the post-#47 `/finish-branch` review that also corrected the registro's UTC-vs-local export and the tamper chain's omission of a correction's reason/actor/till |
