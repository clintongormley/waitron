...
- **Coverage bars:** `@waitron/bookings` takes the six-package bar **98/98/98/95** (data-layer package with its own migration set); `@waitron/server-kit` the **90/90/85/85** floor. `scripts/coverage-thresholds.test.ts`'s hardcoded list gains both, reason in the commit (CLAUDE.md §2).
