...
This is the plan's original draft of the migration. **It shipped, then changed twice under
review** (`02c5f5e`, `29248d6`): the grant narrowed from `SELECT, INSERT` to `INSERT` only (the
by-NIF idempotency rationale below turned out false — RLS hides an existing row from a by-NIF
lookup regardless of the grant, verified live), and a third `ELSIF` was added guarding a
pre-existing LOGIN role, mirroring `credentials_enumerator`'s own guard. The block below is
reproduced from `packages/db/drizzle/0011_provisioner_role.sql` as actually shipped, not the
original draft, so this plan does not re-teach either disproven claim:
...
-- (see above), so a second copy of that grant here would be exactly the redundancy that comment
-- says this design avoids. Changing this grant to `SELECT, INSERT` leaves this file's own test
-- suite green either way — nothing here exercises the difference.
--
