...
- **Never in CI.** The package has **no `test` script and no `*.test.ts` file** — that is what keeps it out of root `pnpm -r test`, the CI shards and the pre-push hook (the pglite bench README explains the mechanism; the same reasoning holds). Its gate is `typecheck` + `format:check` + `lint`; the evidence it works lives in the results note, not in CI.
