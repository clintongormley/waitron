...
- **Never edit a file that calls `startRealPostgres`.** 23 call sites across 22 test files keep their exact import line — `startRealPostgres`, `type RealPostgres` and (in `apps/server`) `roleUrl` all still come from `./testing/postgres.js`. A diff touching one of those files is a design violation; if you believe one needs changing, stop and report it instead.
