...
- **Overtime:** a **resumen handed to the worker with the payslip** → requires a **contracted-hours baseline** on the employment. **Two lawful readings exist and the projection computes BOTH rather than choosing** (landed by the overtime correction, task 3.5):
  - **Daily-accrual (ET art. 35):** a day worked over its ordinary target is an *hora extraordinaria* **that day**, and a shorter later day does **not** cancel it → `Σ over days of max(0, worked(day) − dailyTarget(day))`.
  - **Period-net (ET art. 34.2, *distribución irregular de la jornada*):** hours may net out across days within a reference period — lawful **only** under a convenio or the default 10%-of-annual allowance → `max(0, totalWorked − totalContracted)`.
  - **Which one binds is convenio/contract-dependent — an *asesor-laboral* decision, not a code decision** (see the advisor list below). The code returns both figures side by side plus a per-day worked-vs-contracted breakdown, so any convenio-specific rule is derivable from the data. The **daily target** is a documented floor-scope default (`dailyContractedTargetMinutes` = weekly ÷ a standard working-days count) that D2 scheduling/`convenio_config` will refine; it is not a legal certainty and invents no convenio numbers.
...
- **Which overtime model binds — daily-accrual (art. 35) or period-net / *distribución irregular de la jornada* (art. 34.2)** — and, if irregular distribution is permitted, its reference period and compensation window. The projection computes **both** figures and picks neither (task 3.5); the authoritative selection is a **convenio/contract** question the code must not silently decide. This also fixes the **true per-day contracted target** (today a documented floor-scope default, weekly ÷ a standard working-days count) that the daily-accrual figure depends on → refined by D2 `convenio_config`/scheduling.
...
- **`work_sessions`** — derived projection: `id`, `tenant_id (RLS)`, `person_id`, `location_id`, `work_date date`, `started_at`, `ended_at`, `break_minutes integer`, `worked_minutes integer`, and overtime as **two figures, not one** — `daily_accrual_overtime_minutes` and `period_net_overtime_minutes` (art. 35 vs art. 34.2, above) — never a single collapsed `overtime_minutes`. The pay-period summary the export renders (`PeriodSummary` / `summarisePeriod`) already returns both plus a per-day worked-vs-contracted breakdown and an explicit, convenio-selectable headline; a materialised `work_sessions` table (deferred — the projection is computed in TypeScript today) carries the same both-figures shape. **Mutable/rebuildable** (app role full DML) — it is *not* the legal record (`time_entries` is); it is the queryable/exportable view, reprojected as corrections arrive, latest-correction-wins, rebuildable from `time_entries`.
...
| **2 — D1a Time & attendance (THE LEGAL FLOOR)** | `employments` + `time_entries` (append-only, **role-revocation floor**) + clock in/out/break + `work_sessions` projection (worked + **both** overtime models vs contracted, per-day breakdown) | hand-written immutability SQL (REVOKE + `reject_mutation`/block-truncate triggers, reusing core functions); `WorkforceBackend` clocking methods; projection engine (daily-accrual **and** period-net overtime, §2.1 — the binding model is convenio-selectable, not decided in code; task 3.5); `attendance.*` codes; teeth-tests incl. the daily/period divergence | Slice 1 |
...
- work-session projection + overtime computation (BOTH daily-accrual and period-net, §2.1) — pure logic over `time_entries`, so PGlite/direct unit tests are the right target (no privileges/RLS/concurrency).
- `verifyPin` hashing.
...
- **the two overtime models diverge** — a 9h day then a 7h day against an 8h daily target is **1h daily-accrual** but **0 period-net** (a test where the two figures cannot disagree proves nothing, `CLAUDE.md` §1); both clamps proven by deletion;
