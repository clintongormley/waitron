...
| D8 | **Difference method for VAT rounding.** `total = Σ gross_line` (the customer pays exactly the marked/weighed gross); per rate group `cuota = gross_group − base_group`. This is the legally standard reverse calculation for VAT-inclusive Spanish retail. Its cost — `cuota` can differ from `base × rate` by a rounding céntimo per rate group, and (because per-line bases are rounded then summed) by *more* across many same-rate lines. The primary reconciliation `ImporteTotal == Σ(base+cuota)` and `BaseImponible == Σ line bases` always hold exactly; whether the residual `cuota`-vs-`base×rate` gap stays within AEAT's per-record tolerance is an **external claim not yet confirmed on primary source (asesor / XSD — see §8, backlog)**, not asserted here. It is why the *multiplicative* method was rejected: it would charge a céntimo over the marked price on boundary values. |
...
`base_group × rate` by a rounding céntimo per rate group (more across many same-rate lines, since
per-line bases are rounded then summed) — the residual the difference method accepts in exchange for
charging the exact gross. Whether that residual stays within AEAT's per-record tolerance is left to
asesor / XSD confirmation (§8), not asserted; the two exact invariants above are what this slice
guarantees. Worked example, 0.320 kg @ €24.90/kg incl. 10%:
