...
| N5 | **Single-app kiosk lockdown / MDM-managed handheld** | A device locked to a single app is managed at the OS/MDM layer | **Resolved 2026-09-08 — not a native reason.** Every platform locks a device to a browser at one URL (Guided Access / Single App Mode, Android app pinning / Android Management API dedicated device, Fully Kiosk, Chromium `--kiosk` on a Linux box); the web app needs only a manifest, a wake lock and its own PIN lock. Survey + owner decision (kiosk is optional, never required) in [2026-09-08-handheld-app-store-and-kiosk-findings.md](2026-09-08-handheld-app-store-and-kiosk-findings.md) §2 | Quality/ops, not a payments blocker |
