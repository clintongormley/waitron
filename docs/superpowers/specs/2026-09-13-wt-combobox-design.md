...
| `countLabel` | `(count: number) => string` | Only read when `multiple` is true and more than one value is selected. No default: the primitive never generates its own English copy, the same rule that keeps `wt-data-table`'s `emptyMessage`/`loadingMessage` and `wt-spinner`'s `label` consumer-supplied. An omitted `countLabel` with a multi-selection is a consumer bug, not a fallback case to design for. |
| `noResultsLabel` | `string` | Shown in the panel in place of the option list when the filtered list is empty and `allowAdd` is false (with `allowAdd` true, the add row itself communicates "nothing matched, but you can create it," so no separate empty-state text renders). Same no-default rule as `countLabel`. |
