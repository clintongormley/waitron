...
**2026-09-18, what the prototype measured, and what it left open.** S3
(`bench/sqlite-failover/src/scenarios/s3_copied_replica.ts`, plan Task 8) ran that obligation against
litestream v0.5.17 and a MinIO pin. The byte-identical half holds: a mirroring copy-up restored the
same file, the same rows and the same chain tips as the direct stream. What "must propagate deletions"
actually buys is narrower than the sentence above reads, because the two failing cases behave
differently. Copying additively into a destination that holds ANOTHER node's replica restored that
other node's ledger — litestream exited 0 and raised nothing, and what caught it was SQLite calling the
restored file damaged. Copying additively into a destination holding only the SAME lineage's files,
including objects the source had already compacted away, did NOT diverge at all: the restore came back
identical. A replica file's name carries the transaction range it covers, so putting a file back under
a name it already had puts the same range back twice. **What S3 says it did not test** is whether
litestream ever writes different bytes under a key it has already used; until somebody runs that, the
same-lineage case is measured-safe on this pin rather than known-safe. The practical rule is unchanged
— mirror, do not accumulate — but the reason to insist on it is the foreign-replica case, which is
silent.
...
8. **The on-prem-mirror copy-up must propagate deletions** or a cloud restore from it silently diverges
   (§4.4) — the prototype's explicit check. *(2026-09-18, run: "silently" is right and "diverges" is
   the mild reading. An additive copy into a destination holding another node's replica restored THAT
   node's ledger, litestream exiting 0 with no error — the divergence is a different venue's books,
   not a corrupt file, and only SQLite's own integrity check refused it. The same copy into a
   destination holding only the same lineage's files did not diverge at all. S3 did NOT test whether
