...
1. **Reach latest.** A follower stops following; a fresh instance restores the current generation. If
   ...
   `PRAGMA integrity_check` and the existing hash-chain verifier over the tail of the fiscal ledger. Then reset EVERY inherited in-flight submission (`envios.estado = 'enviando'`) back to `pendiente` — unconditionally, with no staleness cutoff — because nothing is filing yet (§5.2 (b)): the old primary is decommissioned and this node has not begun, so no submission is genuinely in flight, and the duplicate check resolves any the dead primary had actually filed. This runs before the first drain pass; after it, this node's own `enviando` rows are live and the ordinary five-minute `recoverStaleClaims` governs them.
