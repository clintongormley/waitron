...
| D3 | **Packaged as an optional module — `@waitron/recipes`.** Depends on `@waitron/catalogue` + `@waitron/db`; no production-runtime package depends on it (the only consumer is the `demo:recipes` script in `apps/server`, which imports it to illustrate the API — a dev/demo dependency, not a runtime path). With zero recipe rows, catalogue/products/sales behave exactly as today (the shipped #65 direct-tag path is untouched). The module only *augments* — it supplies a derived allergen floor that catalogue publishes. |
