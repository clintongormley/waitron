...
> 2026-09-25: as built (Task 9c), the option is a card beside the "Restore from backup" card on
> the wizard's "Join or recover an existing restaurant" screen. Step 3 also stops when the check
...
> nothing is re-issued or signed. The same holds for a copy whose document cannot be read or is not
> shaped as a list of machines (A53, 2026-09-26). See `docs/backlog.md`'s Task 9a entry.
