...
- “Use another account”, and its replacement icon beside the displayed email, clear saved email and method as well as the current attempt. Refresh must stay on blank email entry. Forgetting must not depend on the displayed email matching an older saved identity. Cancelling or completing an activation link preserves unrelated saved preferences because that flow has no Remember choice.
...
Use one recovery entry point: pending accounts receive a setup link, active accounts receive a reset link, and every address receives the same public acknowledgement. Remove manual invitation-code entry, its unused public API/client helpers, and invitation-code instructions from email. Setup and reset accept token links; only email changes issue short codes. The server still chooses the appropriate action internally and preserves its authorization, account-status rules and token lifetime. Do not expose the selected purpose in the public request response.
