 ...
 * stub stands in for `TillApi.getLocales`. axe is run against the themed host in BOTH themes so the
 * color-contrast check means what it means in the app. Two surfaces are covered: COLLAPSED (only the
 * `aria-haspopup`/`aria-expanded` trigger) and OPEN (the `role="menu"` of native
