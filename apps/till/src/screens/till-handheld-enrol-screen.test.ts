...
import { TillHandheldEnrolScreen } from "./till-handheld-enrol-screen.js";
...
/** Sets the code field's live value and clicks Set up — mirrors a waiter typing a code then tapping. */
function submitCode(el: TillHandheldEnrolScreen, code: string): void {
...
describe("till-handheld-enrol-screen", () => {
  it("registers as a custom element", () => {
    expect(customElements.get("till-handheld-enrol-screen")).toBe(TillHandheldEnrolScreen);
  });
  ...
  it("enrols on a submitted code and signals success", async () => {
    ...
    const done = new Promise<void>((resolve) =>
      el.addEventListener("handheld-enrolled", () => resolve()),
    );
