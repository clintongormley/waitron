 ...
 * (the `<select>` via `aria-labelledby` pointing at the code's name span, the source input a
 * `wt-input` label), and the whole thing is run against the themed host in BOTH themes so a
...
describe.each(["light", "dark"] as const)("allergen-picker a11y (%s theme)", (theme) => {
  it("renders accessibly with the controls enabled", async () => {
    const { el, host } = await mountWidget<AllergenPicker>("dashboard-allergen-picker", {}, theme);
