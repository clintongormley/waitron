...
/**
 * It invokes NEITHER `setLocale` NOR the preference-write endpoint: the parent owns what a pick means.
 *
 * The options are NATIVE `<button role="menuitemradio">` elements as DIRECT children of the
 * `role="menu"` container, so the role + `aria-checked` state land on the element the screen reader
 * actually reaches (a `wt-button` does not forward a role or `aria-checked` to its inner button).
 */
@customElement("dashboard-language-chooser")
  ...
  async #toggle(): Promise<void> {
    if (!this.open && this.locales === undefined) {
      try {
        ...
        // Leave `locales` unset (a later open re-fetches) and `open` false.
        return;
      }
    }
    this.open = !this.open;
...
declare global {
  interface HTMLElementTagNameMap {
    "dashboard-language-chooser": LanguageChooser;
  }
}
