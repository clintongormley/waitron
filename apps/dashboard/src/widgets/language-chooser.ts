...
/**
 * The dashboard's LANGUAGE CHOOSER: a collapsed trigger showing the current language which, on
 * activation, fetches the offered languages and shows them as a menu; picking one emits a composed,
...
export class LanguageChooser extends LitElement {
  ...
  /** Fetch the offered languages. The app injects an adapter over `DashboardApi.getLocales`; the
   * widget calls it at most once (see {@link locales}). */
