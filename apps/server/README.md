...
It reads its own boot env — `DATABASE_URL`, `WAITRON_MIGRATIONS_DATABASE_URL`, the five
`WAITRON_TILL_*_ID`, and (to reach the carrier's replication slot) the three `WAITRON_REPLICATION_*`
variables in the table below. Ordered guards refuse LOUD rather than wipe a box that is not safe to wipe:
...
- **`rejoin.no_carrier`** — the held membership chart names no serving-primary to re-adopt from, so there
  is nowhere to rejoin. `--accept-loss` does **not** waive this.
- **`rejoin.carrier_attached`** — the carrier's replication slot is still `active` (the drain window is
  open); wiping now would lose the tail the carrier has not yet drained.
- **`rejoin.not_drained`** — the carrier has not confirmed it holds this box's tail (its
  `confirmed_flush_lsn` has not reached `deployment.fence_lsn`).

`--accept-loss` waives **only** the last two (`carrier_attached` + `not_drained`): the operator forces the
wipe on a dead or never-fenced box (no fence LSN, or the carrier unreachable), accepting the loss of any
tail that never reached the carrier. It is never a substitute for waiting out a drain that is simply still
in progress. The wipe is irreversible (CLAUDE.md §5), and rejoin performs no replication-slot drop of its
own — the `DROP DATABASE … WITH (FORCE)` reclaims the now-inactive slot with the database.
...
| `WAITRON_MIGRATIONS_DATABASE_URL`          | no       | `DATABASE_URL`                             | The connection migrations run over. See grants above. Native replication (below) reconciles its publications over this SAME connection, because that runs as the table OWNER (`CREATE PUBLICATION … FOR TABLE` is owner-only).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `WAITRON_REPLICATION_PASSWORD`             | no       | — (replication off)                        | The `waitron_repl` login's password. Set together with `WAITRON_REPLICATION_HOST` to turn native logical replication ON; leaving EITHER unset (absent or blank) leaves it off — a blank value never means "no auth". The `waitron_repl` role, `wal_level=logical` and `track_commit_timestamp=on` are a SUPERUSER/box-image bootstrap the app only verifies, never provisions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `WAITRON_REPLICATION_HOST`                 | no       | — (replication off)                        | The host a peer dials to reach THIS node's publisher. Set together with `WAITRON_REPLICATION_PASSWORD` (see above). Absent or blank → replication off.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `WAITRON_REPLICATION_PORT`                 | no       | `5432`                                     | The port for `WAITRON_REPLICATION_HOST`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
