...
add for this table. An UNSTAMPED database (every one that predates this feature, and every one
`stampDeployment` has not been run against yet) reads as `null` and boots regardless; only a database
stamped for the OTHER environment refuses.
