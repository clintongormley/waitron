...
// Reuse dev-setup's building blocks rather than re-implement them: the connection poll and the
// venue probe are identical needs, and one copy keeps the two scripts' behaviour from drifting.
import { DEV_DATABASE_URL, inspectVenues, waitForPostgres } from "./dev-setup.js";
...
export function renderSetupEnvFile(env: SetupEnv): string {
  ...
  const lines = SETUP_ENV_KEYS.map((key) => `${key}=${env[key]}`);
  return [header, ...lines].join("\n") + "\n";
}

export interface DevOnboardOptions {
  /** The database to migrate and to write as `DATABASE_URL`. */
  databaseUrl: string;
  /** Where to write the setup-mode `.env`. */
  envPath: string;
