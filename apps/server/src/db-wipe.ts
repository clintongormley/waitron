 ...
 * migrators would no longer be serialised (`packages/migrations/src/apply.ts`). `venue.lock` stays
 * for the same reason: unlinking it while held let another process take it at once (measured,
 * `docs/developers/conventions-data.md`, "One process per venue folder"). The wipe runs under that
 * lock (`rejoin-command.ts`).
 *
