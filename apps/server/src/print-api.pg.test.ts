...
// two agents contend is proven separately in packages/printing's runtime.race.test.ts, and the lock
// clause it rests on by deletion in packages/db's job-claim.pg.test.ts; the SERVER path calls that
// same `claimPrintJobs`.
