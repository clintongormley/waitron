...
// WHERE filter — PGlite would exercise the query logic too — but this is a tenant-isolation-adjacent
// SECURITY fix, so it is proven against the real cluster the sibling `*.rls` suites use, as the
// non-superuser app_user under RLS: a same-tenant caller must never reach ANOTHER location's rows.
...
/** ONE tenant, TWO venues (locations A and B) — the cross-LOCATION, same-TENANT shape RLS cannot catch
 *  (RLS scopes by tenant, so both venues' rows are visible to the app role). Returns a full TillConfig
 *  scoped to each. */
