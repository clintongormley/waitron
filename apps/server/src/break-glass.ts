...
// Only the scrypt verifier is ever persisted (deployment singleton); the raw secret is returned
...
export async function mintBreakGlassSecret(ownerDb: Database): Promise<string> {
  ...
  await withTransaction(ownerDb, (tx) => setBreakGlassVerifierTx(tx, verifier));
  return secret;
}
