...
// sync_log THROUGH B's outbound tunnel, applies it as the non-superuser sync roles,
...
//   * boot.mirror.test.ts (Task 5) — boots apps/server in mirror mode (read-only gate + ambient
...
//
// Real Postgres × 3 (a mirror applies as a non-superuser role holding only its grants — PGlite
// connects as a superuser, so it would be a false
...
// identically on all three clones (boot.mirror.test.ts's shape) so a booted mirror's reads resolve
...
/** An OS-assigned free port, released before use (boot.mirror.test.ts's helper — WAITRON_HTTP_PORT
...
beforeAll(async () => {
  // The migrations root, built exactly as boot.mirror.test.ts does: boot's from-source default does
