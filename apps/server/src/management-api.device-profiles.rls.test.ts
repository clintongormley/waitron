...
// reads/writes `device_profiles` under FORCE ROW LEVEL SECURITY — both false passes on PGlite's
// superuser connection (CLAUDE.md §4). The same real-Postgres justification and harness as the
// sibling `management-api.canvases.rls.test.ts` (`applyVenue`/`planVenue` + password `login`).
const LOCALE = "es-ES";
 ...
 *  to is never mistaken for a default. Mirrors `management-api.canvases.rls.test.ts`'s helper. */
...
function mountApp(tenantId: string): Hono {
  ...
  mountManagementApi(
    app,
    {
      ...
      // management tests (management-api.canvases.rls.test.ts, …-status/-passkey).
