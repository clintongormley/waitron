...
// Real Postgres, not PGlite: the seal runs on the OWNER connection and the read as `app_user`, a
// two-role split PGlite's superuser-only connection cannot model — it holds every grant, so it could
// not show that the app_user read is permitted at all. The cross-box negative is a property of
// AES-256-GCM authentication, not of the DB, but it is proven here on the same real-role path the
// seal/read take. CLAUDE.md §4.
