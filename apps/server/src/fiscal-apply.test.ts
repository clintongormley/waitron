...
// + target), driven over a real Hono `app.request` wire exactly like sync-e2e.test.ts, whose
// two-clone + syncPullOnce shape this file follows. PGlite would be a false pass (CLAUDE.md §4): the
// apply role is a non-superuser, the immutability guards are grant + trigger, and PGlite connects as
// a superuser that holds every privilege.
//
