...
// migrated here. Shape mirrors move-merge.pg.test.ts's merge/pay deadlock-safety test.
const LOCALE = "es-ES";
...
function systemClock(): TrustedClock {
  return {
    ...
    anchor: () => {
      throw new Error("split-bill.pg.test: anchor() is not used by recordSale");
    },
    currentAnchor: () => null,
  };
}
...
// so each provisioned venue needs its own NIF — the same shape move-merge.pg.test.ts uses.
let nifCounter = 0;
...
beforeAll(() => {
  ...
  backend = new VerifactuBackend({
    ...
    resolveClient: () =>
      Promise.reject(
        new Error("split-bill.pg.test: resolveClient must never be called by recordSale"),
      ),
  });
});
