...
/** The floor for an envelope passphrase. The envelope wraps an unrecoverable secret, so a weak
 * passphrase is the whole risk surface; the 12-char floor is this build's choice, not a
 * spec-mandated number. */
export const MIN_PASSPHRASE_LENGTH = 12;

const ENVELOPE_VERSION = 1;
// Bounds an UNTRUSTED envelope's KDF cost so a hand-edited envelope cannot make decrypt allocate wildly.
// decryptSecretEnvelope derives with the ENVELOPE's OWN recorded kdf.N/r/p (self-describing, so an
// envelope stays decryptable after SCRYPT_PARAMS is later hardened — see decryptSecretEnvelope), so
// this bound is what actually caps the real scryptSync call's memory use, not just the envelope's
// shape. The operator runs decrypt on their own envelope, so this is defence-in-depth, not a security
// boundary.
const MAX_SCRYPT_N = 2 ** 20;
// Bounds an UNTRUSTED envelope's ciphertext so a hostile envelope cannot make decrypt allocate a huge
// Buffer (from `env.ct`) BEFORE the GCM auth failure. Callers wrap only small secrets, so 1 MiB is
// very generous.
...
function parseEnvelope(envelopeJson: string): Envelope {
  ...
  // Reject an over-large ciphertext on the base64 STRING length, BEFORE decryptSecretEnvelope's
  ...
  if (
    ...
    // ERR_CRYPTO_INVALID_SCRYPT_PARAMS out of scryptSync — which decryptSecretEnvelope's try/catch
    // turns into our contract error. That try/catch, not this bound, is what guarantees no raw 500.
    // These values genuinely reach scryptSync (decryptSecretEnvelope derives with the envelope's own
    // kdf.N/r/p), so unlike a pure shape check, getting this bound wrong lets a hostile N actually
    // reach scryptSync.
...
export function decryptSecretEnvelope(envelopeJson: string, passphrase: string): string {
  ...
  try {
    // Derive with the ENVELOPE's own recorded cost, not the compiled default: the wrapped artifact is
    // external and meant to survive indefinitely, and it stays decryptable after SCRYPT_PARAMS is
    // hardened only if decrypt honours whatever cost it was actually sealed under.
    ...
    // passphrase_invalid: this is a malformed envelope, not a failed authentication.
    throw new AppError("recovery.bundle_invalid", { reason: "malformed" });
  }
  try {
    // GCM authentication failed: wrong passphrase OR tampered envelope — deliberately one code.
