...
/**
 * Resolves the request's cookie to an OPEN shift session, or throws `session.required`. The
 * id-only lookup assumes one tenant per database. This is validation against the database, not a
 * presence check: the cookie's id is looked up as the app role under the till's tenant with
 * `ended_at IS NULL`, so an absent id and an already-logged-out session both fail exactly as a
