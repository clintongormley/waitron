...
describe("startServer, against a real container as the deployment role", () => {
  ...
  it("setup mode: a CLOUD node takes the vault key from the environment — no secrets.env on disk, and the env ring seals a credential", async () => {
    // The §4 security ceiling (Task 12): on a cloud node the platform's secrets service injects
    // WAITRON_CREDENTIALS_KEY into the environment; it is written to NO disk file, so a snapshot/dump/
    // backup yields no usable key. Boot's setup branch builds the ring ENV-FIRST, so provisioning can
    // seal the primary node's membership private key with no `secrets.env` present at all — RED before
    // this change, where the setup branch `readFileSync(secrets.env)`s unconditionally and ENOENTs.
    // Modelled on the demo-provision test above; the seal proof is the membership trust anchor it
    // establishes (Slice 4), driven by boot.ts's real `db: ownerDb` + `ring` injection.
    const port = await freePort();
    const stateDir = await mkdtemp(join(tmpdir(), "waitron-boot-cloud-envkey-state-"));
    const handle = resolveSharedHandle(undefined);
    const pg = await cloneTemplate(handle.uri, pickTemplate(handle, "manifest"), nextCloneName());
    const check = await pg.connect();
    try {
      await withMockedKill(async (kills) => {
        const server = await startServer({
          ...
          WAITRON_STATE_DIR: stateDir,
          WAITRON_ENV: "preproduction",
          // The platform-injected vault key — the cloud node's WHOLE point.
        ...
        const { via, close } = httpsVia(ca);
        try {
          // The key never touched disk: the TLS quartet is present, but there is no secrets.env.
          expect(existsSync(join(stateDir, "tls", "server.key"))).toBe(true);
          expect(existsSync(join(stateDir, "secrets.env"))).toBe(false);

          const body = { mode: "demo", venue: provisionVenueBody("60000015K") };
          const response = await fetch(`https://127.0.0.1:${port}/setup-api/provision`, {
            ...via,
            method: "POST",
            headers: { "content-type": "application/json" },
          ...
          // The env ring sealed the node's membership private key end-to-end: the public half is
          // stamped and the venue has exactly its one trust anchor. A ring that failed to build (or
          // built from a wrong key) could not have sealed it.
          const trust = await readMembershipTrustSet(check, json.tenantId);
          expect(Object.keys(trust)).toHaveLength(1);
          expect(Object.values(trust)[0]).toMatch(/.+/);
          // Still no secrets.env after a full provision.
          expect(existsSync(join(stateDir, "secrets.env"))).toBe(false);
  ...
  it("setup mode: an env vault key that DIFFERS from an on-disk secrets.env refuses to boot (server.credentials_key_conflict)", async () => {
    // The single precedence guard: env key wins, but a usable env key AND a DIFFERING secrets.env must
    // not both be honoured — sealing under one ring and reading under the other would strand every
    // credential sealed under the key we did not use. Boot refuses loudly instead. A pristine manifest
    // clone is migrated so boot reaches the setup-branch ring load where the guard lives.
    const port = await freePort();
    const stateDir = await mkdtemp(join(tmpdir(), "waitron-boot-keyconflict-state-"));
    const handle = resolveSharedHandle(undefined);
    const pg = await cloneTemplate(handle.uri, pickTemplate(handle, "manifest"), nextCloneName());
    try {
      // A DIFFERENT key already on disk (an on-prem-minted secrets.env), from the env key below.
      writeFileSync(
        join(stateDir, "secrets.env"),
        `WAITRON_CREDENTIALS_KEY=${Buffer.alloc(32, 1).toString("base64")}\n` +
          `WAITRON_CREDENTIALS_KEY_VERSION=1\n`,
        { mode: 0o600 },
      );
      await expect(
        startServer({
          ...
          WAITRON_STATE_DIR: stateDir,
          WAITRON_ENV: "preproduction",
          WAITRON_CREDENTIALS_KEY: Buffer.alloc(32, 2).toString("base64"),
          WAITRON_CREDENTIALS_KEY_VERSION: "1",
        }),
      ).rejects.toMatchObject({ code: "server.credentials_key_conflict" });
    } finally {
      await pg.stop();
      await rm(stateDir, { recursive: true, force: true });
    }
