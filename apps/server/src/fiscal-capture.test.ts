...
// PGlite connects as a superuser and holds every privilege, so its writer never meets the REVOKE ALL
// the capture path must get past — a false pass here (CLAUDE.md §4). The `manifest` template runs the
...
// The app writer (app_login: a non-superuser member of app_user, so the REVOKE ALL genuinely applies
// to it). One connection is opened for the whole file and reused by `asWriter`,
