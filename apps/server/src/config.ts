...
export function loadMirrorSyncConfig(env: Env): SyncTransportConfig {
  return {
    ...
    // — a mirror is always `secondary` and holds no `sync_log` to prune), but the shared
    // `SyncTransportConfig` shape requires it; defaulted so the type is honest rather than asserted.
    retentionTickMs: positiveInt(
      env,
      "WAITRON_SYNC_RETENTION_TICK_MS",
      DEFAULT_SYNC_RETENTION_TICK_MS,
