...
// settle) run as the deployment role holding only its grants, and the P1(commit)→collect→P3 ordering plus the
...
// those grants the ceiling (the provider's own `collect` does NOT `SET ROLE app_user`; it runs as
// whatever role holds the handle). Mirrors `payments-stripe`'s stripe suite.
...
// probe role the package globalSetup creates (shared with till-api), in place of the per-file
