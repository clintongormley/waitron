...
// real-Postgres two-clone harness as fiscal-apply.test.ts (Task 6) and fiscal-upsert.test.ts
// (Task 7): two `manifest` clones (source + target) driven over a real Hono `app.request` wire, applied
// as the non-superuser `sync_applier`, whose grants PGlite's superuser would hold regardless
// (CLAUDE.md §4).
//
