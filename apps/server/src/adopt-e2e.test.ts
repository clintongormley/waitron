...
// dashboard GET returns end-to-end (mirror-e2e.test.ts's choice, reused).
 ...
 * in as `ownNodeId`. The vault key is the same RING adopt sealed under. This is mirror-e2e.test.ts's
...
function adoptDeps(overrides: Partial<AdoptDeps> = {}): AdoptDeps {
  return {
    ...
    // Echoed by `provisioning.foreign_tenant` if a foreign bundle is adopted into an occupied
...
beforeAll(async () => {
  ...
  const venue = await applyVenue(
    ...
    { db: primary.admin, modules: ALL_MODULES },
  );
  designated = {
  ...
  // The box's HTTPS sync-api behind the tunnel (mirror-e2e.test.ts's shape): a leaf whose SAN is
  ...
  // (global fetch) reaches it (mirror-bundle-api.test.ts mounts the same endpoint). Its stateDir
