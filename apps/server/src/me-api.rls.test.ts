...
//  1. TENANT ISOLATION: a management-session cookie is scoped to its tenant by RLS on the app role —
//     used against another tenant's app it names no visible session and 401s. The planning reads are
//     also isolated.
...
describe("Me API over real Postgres (RLS + the identity property)", () => {
  it("files a swap as the SESSION's person, never the body's — even when the body names someone else", async () => {
    ...
    // TESTCONTAINERS_RYUK_DISABLED=true): P is signed into the dashboard; the request body hostilely
    // names Q as `requestedByPersonId`. The route ignores the body and files as P (the session), a 201
    // with `requested_by_person_id = P`. Changing me-api.ts's `requestedByPersonId: personId` to read
  ...
  it("isolates tenants — a tenant-A session is rejected against tenant B, and never leaks A's rows", async () => {
    // A management-session cookie is scoped to its tenant by RLS on the app role
    // (resolveManagementSession's app_user lookup). Used against a different tenant's app it names no
    // visible session → 401. Removing asAppUser would let the superuser find the foreign session and
    // 200 — the load-bearing asAppUser differential.
    const venueA = await setupVenue();
    const venueB = await setupVenue();
    const pA = await seedPerson(venueA.tenantId, "P-A");
    ...
    const cookieA = await cookieFor(venueA.tenantId, pA);
    ...
    const ownRead = await send(
      appA,
      "GET",
      "/management-api/me/schedule/shifts?from=2026-07-01&to=2026-07-08",
      cookieA,
    );
    expect(ownRead.status).toBe(200);
    expect(((await ownRead.json()) as { id: string }[]).map((r) => r.id)).toEqual([aShift]);

    // A's session on B's app: the session belongs to tenant A, RLS-hidden under B → management_session.required.
    ...
    const pB = await seedPerson(venueB.tenantId, "P-B");
    const cookieB = await cookieFor(venueB.tenantId, pB);
