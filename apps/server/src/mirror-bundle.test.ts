...
// A mutable copy, so one case can add a module for its own duration.
...
// Never stamped, for `mirror.not_provisioned`.
...
let caPem: string;
let db: Database;
// Provisioned once: the suite does not reset between tests, and a database holds one tenant.
