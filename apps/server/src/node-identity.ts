 ...
 * the owner path: `nodes` is one of the tables `scripts/write-path-tables.json` lists, which a
 * request path may read and never write. Nothing in the engine enforces that: there are no roles
 * and no grants, and `ownerDb` and the app handle are the SAME handle at runtime. What keeps this
 * write in a named file is `scripts/write-path-tables.test.ts`, which reads the tree as text. Runs
 * AFTER provisionVenue mints the node row the stamp updates.
 */
