...
- **A restore takes one of two sources.** `restore <backup-file>` restores an encrypted backup
  ...
  `restore.membership_invalid` when the restored copy's list of machines cannot be read, or does
  not pass its signature check against the node keys in that same copy. A box that comes up fenced
  or as a mirror runs only the first of those checks, and one still waiting to finish an adoption
  runs neither. The copy is left unchanged,
...
next start tries again. A restored copy whose list of machines cannot be read or fails its
signature check stops the start before the leaf is replaced (`restore.membership_invalid`, described under
