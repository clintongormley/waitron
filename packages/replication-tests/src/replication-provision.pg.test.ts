...
// A SMALL hardcoded real-table list, not the lists derived from @waitron/composition. Every one is
// created by the core baseline and classified in S1: `sales` and `tenders` are ledger, `tenants` and
// `locations` are state.
