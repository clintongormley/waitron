 ...
 *
 * `connect()` returns the container's default (superuser) connection — the payments RLS suite uses
 * it to SEED rows with RLS bypassed — while `connectAs(role, password)` returns a fresh connection
