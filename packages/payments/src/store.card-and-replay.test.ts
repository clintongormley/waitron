...
describe("findCapturedPaymentForWorkingOrder", () => {
  it("returns saleId once associated — the replay branch, not just the resume (saleId null) one", async () => {
    // seedSale plants a real committed sale + covering tender, the minimal thing
    // associatePaymentWithSale needs to point a payment at (no need to go through @waitron/core's
    // full recordSale here).
...
describe("payments card columns", () => {
  ...
  // Both refusals below keep the assertion they carried on PostgreSQL, unchanged: SQLite names a
  // CHECK constraint in its message (`CHECK constraint failed: payments_card_last4_ck`), so
  // `/card_last4|check/i` and `/card_entry_mode|check/i` still match what the engine says.
