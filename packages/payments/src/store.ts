 ...
 * before and after.
 *
 * Shares its predicate with `listAcceptedOffline` through `forwardableWhere`. A REAL adapter uses
 * `listAcceptedOffline` instead, so nothing of its transaction is held across the processor
