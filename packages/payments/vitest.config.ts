...
export default defineConfig({
  test: {
    ...
    // under-merges BRANCH coverage across fork workers, so a branch this package's tests cover in
    // one worker but not another is reported uncovered after the merge — proven, store.ts is 100%
    // branch when store.test.ts runs alone but 83.78% in the parallel full run, and single-fork
    // restores it to 100%.
    ...
    // running the test files back-to-back rather than in parallel (a few seconds on a suite this
    // size).
    maxWorkers: 1,
    coverage: {
      provider: "v8",
