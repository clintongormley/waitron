...
// any of this. sessions grants app_user exactly SELECT, INSERT, UPDATE (drizzle/0003_sessions_rls.sql):
// a missing grant, or a DELETE grant that should not be there, is invisible under PGlite.
...
async function seedTillAndPerson(
  db: Database,
  tenant: string,
): Promise<{ tillId: string; personId: string }> {
  const location = await db.execute<{ id: string }>(sql`
    insert into locations (tenant_id, name, invoice_locales, operation_description)
    values (${tenant}, 'Main', array['en'], 'Sale on premises') returning id`);
  const locationId = location.rows[0]!.id;
  const till = await db.execute<{ id: string }>(sql`
    insert into tills (tenant_id, location_id, name)
    values (${tenant}, ${locationId}, 'Till 1') returning id`);
  const person = await db.execute<{ id: string }>(sql`
    insert into persons (tenant_id, display_name, pin_hash)
    values (${tenant}, 'Ana', ${hashPin("1234")}) returning id`);
  return { tillId: till.rows[0]!.id, personId: person.rows[0]!.id };
}
...
describe("sessions under real row-level security", () => {
  it("isolates a session by tenant: another tenant sees none, the owning tenant sees its one", async () => {
    ...
    const { tillId, personId } = await seedTillAndPerson(suite.admin, tenantA);

    // Open a session for tenant A as the superuser (RLS bypassed), so there IS a row to hide.
    await suite.admin.execute(sql`
      insert into sessions (tenant_id, person_id, till_id)
      values (${tenantA}, ${personId}, ${tillId})`);
    ...
    const seen = await suite.admin.execute<{ count: string }>(
      sql`select count(*) as count from sessions where tenant_id = ${tenantA}`,
    );
    ...
    try {
      ...
      // sessions_tenant_isolation's USING to `true` leaks tenant A's row here (the deletion-proof).
      const cross = await withTenant(probe, tenantB, (tx) =>
        tx.execute<{ id: string }>(sql`select id from sessions`),
      );
      ...
      const own = await withTenant(probe, tenantA, (tx) =>
        tx.execute<{ person_id: string; till_id: string; ended_at: string | null }>(
          sql`select person_id, till_id, ended_at from sessions`,
        ),
      );
      expect(own.rows).toEqual([{ person_id: personId, till_id: tillId, ended_at: null }]);
    } finally {
      await probe.close();
    }
  });
  ...
  it("returns nothing at all with no tenant GUC set", async () => {
    ...
    const { tillId, personId } = await seedTillAndPerson(suite.admin, tenant);
    const probe = await suite.pg.connectAs(PROBE_ROLE, PROBE_PASSWORD);
    try {
      await withTenant(probe, tenant, (tx) =>
        tx.execute(sql`
          insert into sessions (tenant_id, person_id, till_id)
          values (${tenant}, ${personId}, ${tillId})`),
      );
  ...
  it("lets app_user stamp ended_at on its own tenant's session — sessions is MUTABLE", async () => {
    // The mutable half of the grant. A session is closed by stamping ended_at on logout, not by
    // deleting the row, so UPDATE must succeed as the probe role. Removing UPDATE from
    // 0003_sessions_rls.sql's GRANT fails this with 42501 (permission denied for table sessions).
    const tenant = await seedTenant(suite.admin);
    const { tillId, personId } = await seedTillAndPerson(suite.admin, tenant);
    const probe = await suite.pg.connectAs(PROBE_ROLE, PROBE_PASSWORD);
    try {
      ...
      await withTenant(probe, tenant, (tx) =>
        tx.execute(sql`update sessions set ended_at = now() where tenant_id = ${tenant}`),
      );
      const rows = await withTenant(probe, tenant, (tx) =>
        tx.execute<{ ended: boolean }>(
          sql`select ended_at is not null as ended from sessions where tenant_id = ${tenant}`,
        ),
      );
  ...
  it("refuses to delete a session — DELETE was never granted to the app role", async () => {
    // sessions is mutable but never deleted: a shift login is closed by stamping ended_at, and the
    // cash-up / audit trail that references it must not lose its referent. The grant is exactly
    // SELECT, INSERT, UPDATE, so a DELETE fails with 42501. Adding DELETE to 0003_sessions_rls.sql's
    ...
    try {
      ...
      const error = await withTenant(probe, tenant, (tx) =>
        tx
          .execute(sql`delete from sessions where tenant_id = ${tenant}`)
          .then(() => undefined)
          .catch((e: unknown) => e),
      );
