...
describe("totp", () => {
  ...
  it("rejects a malformed token without throwing", () => {
    // otplib@13.5.0's guardrails THROW on a token that is not six digits ("Token must be 6 digits,
  ...
  it("rejects a malformed secret without throwing", () => {
    // otplib@13.5.0's Base32 plugin THROWS on a non-base32 secret ("Invalid Base32 string"); the
  ...
  it("fails closed when otplib throws on a missing secret", () => {
    ...
    // JSON, `any`) can reach this at runtime. Probed against otplib@13.5.0: a null/undefined/empty
