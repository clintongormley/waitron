...
// { window: 1 }`. Proven against otplib@13.5.0 at a fixed epoch E (30s period): a token minted at
...
export function verifyTotp(token: string, secret: string): boolean {
  try {
    ...
    // THROW. Probed against otplib@13.5.0, verifySync throws on: a malformed base32 secret
