...
import { requestInvitationAction, requestPasswordResetAction } from "./account-action.js";
...
describe("account-action issuance under real concurrency", () => {
  it("serialises replacement on the person row so only the newest action remains live", async () => {
    ...
    try {
      c1Done = withTenant(c1, tenantId, async (tx) => {
        const issued = await requestPasswordResetAction(tx, {
          tenantId,
      ...
      const blocked = await captureError(() =>
        withTenant(c2, tenantId, async (tx) => {
          ...
          return requestPasswordResetAction(tx, { tenantId, email: "reset-race@x.com" });
        }),
      );
      ...
      await withTenant(c2, tenantId, (tx) =>
        requestPasswordResetAction(tx, { tenantId, email: "reset-race@x.com" }),
      );
