...
export default defineConfig({
  test: {
    ...
    // and `--poolOptions.forks.singleFork=false` — the vitest 3 spelling, which vitest 4 no longer
    // has (no module's branch coverage here mis-merges below the gate across workers). So
    // single-fork is not demonstrably load-bearing for identity's threshold
    ...
    // cluster's single 100-connection budget is a non-issue here and needs no `maxWorkers` cap — even
