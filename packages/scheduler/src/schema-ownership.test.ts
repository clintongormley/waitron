...
describe("the scheduler schema entrypoint owns exactly its own tables", () => {
  it("exports no table this package does not own", () => {
    // Not `(v): v is PgTable => is(v, PgTable)`: this barrel also exports `runState`, a plain
    // readonly string tuple, alongside `scheduledRuns` — a specific `PgTableWithColumns<...>`
    // union member that carries more properties than the general `PgTable` class, so an explicit
