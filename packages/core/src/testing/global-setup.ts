 ...
 * non-superuser LOGIN role per suite and connect AS it (`pg.connectAs`); this package's real-PG suites
 * reach the non-superuser path with `asAppUser(tx)`, which `SET ROLE`s the connection to the
 * `app_user` GROUP role CORE's `0001_db_baseline_sql.sql` already creates inside the template. Their
 * two-backend races open the extra backends with `pg.connect()` (superuser connections to the clone)
 * and apply `asAppUser` inside each, so they need no probe role either.
 *
...
export default async function ({ provide }: GlobalSetupContext) {
  const { handle, teardown } = await startSharedContainer({
    dockerRequired:
      ...
      "PGlite serialises every query onto one backend, so their races across two distinct backends " +
      "are a false pass there.",
    templates: {
