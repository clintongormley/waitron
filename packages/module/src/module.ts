...
export interface WaitronModule {
   ...
   * completeness guard checks it covers each module's `CREATE TABLE`s exactly once. No foreign key may
   * join a `local` table to a `ledger`/`state` one: `scripts/two-file-foreign-keys.test.ts`. What each
   * class means is on `TableClass` (`@waitron/sync-enrolment`). */
