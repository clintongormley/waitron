...
/**
 * Mirrors packages/db/src/errors.reachability.test.ts and packages/fiscal/src/
 * errors.reachability.test.ts for the same reason it exists in both: a package that augments
 * `ErrorParams` must keep the augmenting file transitively reachable from that package's own
 * public barrel (`./index.ts`), not merely present somewhere under `src/`.
 *
 ...
 * anything imports it (packages/fiscal-verifactu/tsconfig.json `include`s all of `src`), so
 * nothing that runs from *inside* this package — typecheck, lint, test — would ever catch
 * `./errors.ts` becoming unreachable from `./index.ts`. Only a program that only sees the public
 * barrel (an external consumer of `@waitron/fiscal-verifactu`) would notice, and this repo has no
 * such program running in CI.
 *
 ...
 * `errors.ts`. That is exactly the property `registro-sif.ts`'s side-effect `import "./errors.js"`
