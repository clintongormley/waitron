...
/**
 * Task 3's English-only guard (packages/db/src/english-only.ts) scans the twelve `GENERIC_PACKAGES`
 * ("db", "core", "fiscal", "shared", "payments", "scheduler", "credentials", "workforce",
 * "reporting", "identity", "catalogue", "sync") and explicitly names this package — alongside
...
describe("the English-only vocabulary guard is scoped out of this package", () => {
  it("does not scan fiscal-verifactu", () => {
    expect(englishOnlySource).toMatch(
      /GENERIC_PACKAGES\s*=\s*\[\s*"db",\s*"core",\s*"fiscal",\s*"shared",\s*"payments",\s*"scheduler",\s*"credentials",\s*"workforce",\s*"reporting",\s*"identity",\s*"catalogue",\s*"sync",?\s*\]/,
    );
  });
