 ...
 * The upsert below keeps a higher stored counter; `registro_sif_instalacion_uq` refuses
...
export async function raiseInstallationFloor(
  ...
  await tx
    ...
    .onConflictDoUpdate({
      ...
      set: {
        // Two-argument `max` is a scalar maximum that returns null if either side is null; neither
        // can be here.
