 ...
 * responsibility — core migrations must run before these (the `locations` foreign key) —
 ...
 * AFTER `workforce`, BEFORE `fiscal` — `convenio_config` FKs only core `locations`, so it
