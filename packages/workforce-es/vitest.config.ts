...
export default defineConfig({
  test: {
    globals: true,
    // globalSetup boots ONE shared Postgres container and migrates the `core_identity_workforce_es` template the
    ...
    // The PGlite suites (work-summary.test.ts, convenio.test.ts, migrations.test.ts) boot PGlite (a
    // WASM PostgreSQL) and apply the migration sets, longer than Vitest's 5s default on a cold CI
    // runner; the real-PG (RLS) suite now clones the shared container's migrated `core_identity_workforce_es`
    ...
    // to globalSetup, which vitest does NOT bound by hookTimeout. testTimeout covers a migration suite
