...
export default defineConfig({
  test: {
    globals: true,
    // PGlite boots a WASM PostgreSQL and applies the migration sets; the RLS suite additionally pulls
    // and starts a real Postgres container via Testcontainers. Both costs are one-off, paid in a
    // beforeAll, so hookTimeout is the generous one (a container cold pull on a slow CI runner) while
    // testTimeout covers a migration suite booting a second database inside a single `it`.
    testTimeout: 120_000,
    hookTimeout: 180_000,
