/**
 * RED ON THIS BRANCH, AND NOT BY OVERSIGHT — the per-person `for no key update` on `persons` that each clock method took is gone (clocking.ts's `lockPerson`, deleted with it).
 *
 ...
 * than deleted because its behavioural subject — that two same-person clock-ins cannot both observe `out` and both
 * append an `in` — still has to hold. Note what does NOT carry over: the clock-vs-correction case
 * exists to pin a lock MODE (`for no key update` rather than `for update`, to avoid an ABBA cycle
 * with the `for key share` locks a `time_entries` insert took on its referenced `persons` rows).
 * There is no SQLite version of that question: the engine takes neither lock, and there is one
 * writer. That case is retired by the engine, not moved.
 */
