...
/**
 * Mirrors packages/payments/src/errors.reachability.test.ts (and the copies in db/core/fiscal/
 * credentials) for the same reason it exists in each: a package that augments `ErrorParams` must
 * keep the augmenting file transitively reachable from that package's own public barrel
 ...
 * alone (an external consumer of `@waitron/workforce`) would notice.
 */
