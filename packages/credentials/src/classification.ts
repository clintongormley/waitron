...
const LOCAL =
  "sealed under the key ring of the node that wrote it, so only that node opens a row; in venue.db like every table, and a rebuild from the bucket brings the same node's key back with it (slice-2 spec §3.1). Not keyed by node id: slice 3 moves the vault to a venue key (§3.3)";
