 ...
 * `credentials.test.ts` connects AS it (`pg.connectAs`) so `credential_tenants` is called with the app role's privileges. Roles are CLUSTER-global — a shared container is one cluster, and every
 ...
 * `0001_db_baseline_sql.sql` creates it and `startSharedContainer` runs `roles` AFTER the templates
...
export default async function ({ provide }: GlobalSetupContext) {
  const { handle, teardown } = await startSharedContainer({
    dockerRequired:
      ...
      "the `credential_tenants` function through a non-superuser caller (see " +
