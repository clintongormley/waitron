 ...
 * caller translating one of these into a domain error matches the WORDS.
 *
 * The migrations are SQL and spell each string out. What binds the two is
 * `scripts/behavioural-triggers.test.ts`, which asserts the constants below against the refusals a
 * database the PRODUCT migrated actually raises.
 */
...
/** `tenders_reject_post_settlement`: a tender arrived after the sale was settled. */
