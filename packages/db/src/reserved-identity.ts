 ...
 * `app_user` SELECT alone, and this engine has no roles and no grants — an ordinary insert into
 * `nodes`, and an update of `public_key` or `endorsement`, both succeed (measured 2026-09-23 on
 * Node v26.7.0 against the core migration set). Adopt is the only writer by convention now. Caller
 * supplies a `withTransaction` tx so this commits with the reserved SIF + sealed key in one
 * transaction (CLAUDE.md §3 — a write-path helper takes a `tx`).
 */
 ...
 * and no grants and the update succeeds on an ordinary handle, measured 2026-09-23 on Node v26.7.0
 * against the core migration set. What is still true is the code half: no runtime path retires a
 * series — a restore does, before opening the node's replacement series.
 */
