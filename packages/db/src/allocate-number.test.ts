...
// that skipped the role switch and failed only in production. SQLite has no roles and no grants, so
// that case now duplicates the first one and is deleted. Nothing states which privileges this
// allocation needed.
