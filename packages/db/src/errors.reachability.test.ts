...
/**
 * Mechanical guard for the rule stated in packages/shared/src/errors.ts's design comment: a
 ...
 * anything imports it (packages/db/tsconfig.json `include`s all of `src`), so nothing that runs
 * from *inside* this package — typecheck, lint, test — would ever catch `./errors.ts` becoming
 * unreachable from `./index.ts`. Only a program that only sees the public barrel (an external
 * consumer of `@waitron/db`) would notice, and this repo has no such program running in CI.
 *
 * This is a reachability check, not a type checker: it parses relative `import`/`export …from`
 * specifiers as text (the same technique packages/shared's `conventions.test.ts` uses to read
 * source without evaluating it) and walks the graph from `./index.ts`. It cannot see whether
 * anything actually compiles — only whether some chain of imports, starting at the barrel,
 * mentions `errors.ts`. That is exactly the property `allocate-number.ts`'s side-effect
 * `import "./errors.js"` exists to keep true; deleting that import is the one edit this test
 * exists to catch.
 */
