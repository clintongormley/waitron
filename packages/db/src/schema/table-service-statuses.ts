...
/**
 * A venue-configured MANUAL service status a table may carry — "Bill requested", "Needs
 * cleaning". Venue-wide config, not per-location. One status is set on a table at a time via
 * `dining_tables.status_id`; this table is the authorable SET.
 */
export const tableServiceStatuses = table(
  ...
  (t) => [
    // `createStatus`/`updateStatus` map this refusal to `status.label_taken`.
