 ...
 * always-on kitchen screen — is the ONLY kind wired this slice; it is a pgEnum, not a text check,
 * so trusting the till device itself, or a customer-facing display, is an ADDITIVE enum value later
 * rather than a destructive migration (spec §0, §9). Both `devices` and `device_pairing_codes`
 * carry a column of this type, so drizzle-kit emits `CREATE TYPE device_kind` once.
 */
export const deviceKind = pgEnum("device_kind", ["kds_station"]);
