...
// Real Postgres, not PGlite: this suite proves tenant_themes' tenant-isolation policy as the
// NON-OWNER app role. It writes and reads under withTenant + asAppUser, so the INSERT exercises the
// table-level GRANT (0091) and the WITH CHECK half of tenant_themes_tenant_isolation, and the SELECT
...
describe("tenant_themes under real row-level security", () => {
  ...
  it("isolates a tenant's theme: the owner reads its row, another tenant sees none", async () => {
    ...
    // fails on `relation "tenant_themes" does not exist` — the real cause — rather than a drizzle
    // schema mismatch.
    const own = await withTenant(suite.admin, TENANT_A, async (tx) => {
      await asAppUser(tx);
      await tx.execute(sql`
        insert into tenant_themes (tenant_id, theme)
        values (${TENANT_A}, '{"accent":"red"}'::jsonb)`);
      const result = await tx.execute<{ tenant_id: string }>(
        sql`select tenant_id from tenant_themes`,
      );
      return result.rows;
    });
    ...
    const seenByB = await withTenant(suite.admin, TENANT_B, async (tx) => {
      await asAppUser(tx);
      return tx.execute<{ tenant_id: string }>(sql`select tenant_id from tenant_themes`);
    });
  ...
  it("lets the app role UPDATE its own tenant's theme in place (INSERT then UPDATE grant)", async () => {
    ...
    await withTenant(suite.admin, tenantId, async (tx) => {
      ...
      await tx.execute(sql`
        insert into tenant_themes (tenant_id, theme)
        values (${tenantId}, '{"accent":"blue"}'::jsonb)`);
      await tx.execute(
        sql`update tenant_themes set theme = '{"accent":"green"}'::jsonb where tenant_id = ${tenantId}`,
      );
      const updated = await tx.execute<{ theme: unknown }>(
        sql`select theme from tenant_themes where tenant_id = ${tenantId}`,
      );
      expect(updated.rows).toEqual([{ theme: { accent: "green" } }]);
    });
  });
  ...
  it("refuses an INSERT carrying another tenant's id — the WITH CHECK half of the policy", async () => {
    ...
    // row-level security policy. Deleting the WITH CHECK clause from 0091 lets this write land.
  ...
  it("proves the isolation predicate is load-bearing: neutralise it and A's row leaks to B", async () => {
    ...
    await withTenant(suite.admin, leakA, async (tx) => {
      await asAppUser(tx);
      await tx.execute(sql`
        insert into tenant_themes (tenant_id, theme) values (${leakA}, '{}'::jsonb)`);
    });
    const isolated = await withTenant(suite.admin, leakB, async (tx) => {
      await asAppUser(tx);
      return tx.execute<{ tenant_id: string }>(
        sql`select tenant_id from tenant_themes where tenant_id = ${leakA}`,
      );
    });
    ...
    try {
      await suite.admin.execute(sql`
        alter policy tenant_themes_tenant_isolation on tenant_themes
