...
export default defineConfig({
  test: {
    ...
    // The PGlite suites (ingredients.test.ts, recipes.test.ts) boot PGlite (a WASM PostgreSQL) and
    // apply `@waitron/db`'s migrations, longer than Vitest's 5s default on a cold CI runner; the
    // real-PG (RLS) suites now clone the shared container's migrated `core` template (globalSetup,
