...
opened, and — through the fiscal regime's own venue-field seat — a legal name or operation
...
(`setup.request_invalid`, naming the offending field); a concurrent run that races a conflicting row
is caught as `provisioning.venue_conflict`.
