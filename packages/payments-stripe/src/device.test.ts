...
// core+payments only, no sync capture triggers); threaded into the adapter's withTransaction (design §4d(B)).
...
describe("the stripe on-device adapter against a real database", () => {
  it("forward() clears an offline payment when given the only Database handle the API can build", async () => {
    ...
    // handle" instead, which cannot exist (`withTransaction` scopes transaction-locally, from inside a
