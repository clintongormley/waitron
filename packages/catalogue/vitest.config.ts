...
export default defineConfig({
  test: {
    ...
    // (RLS) suite clones (~26ms) instead of each file booting and migrating its own (~1.5s). See
    ...
    // The PGlite suites (operations.test.ts, integration.test.ts) boot PGlite (a WASM PostgreSQL) and
    // apply `@waitron/db`'s migrations, longer than Vitest's 5s default on a cold CI runner; the
    // real-PG (RLS) suite now clones the shared container's migrated `core` template (globalSetup,
    // above). Each per-suite cost is paid in a beforeAll — the PGlite WASM boot, or the real-PG ~26ms
    // clone — so hookTimeout stays generous for the PGlite boot; the ~26ms clone is a harmless ceiling
    // under it. The container boot / image pull is NOT in a beforeAll: it moved to globalSetup, which
    // vitest does NOT bound by hookTimeout.
    testTimeout: 30_000,
    hookTimeout: 60_000,
