...
export default defineConfig({
  test: {
    ...
    // Task 5's DB-backed suites boot PGlite (a WASM PostgreSQL) and apply `@waitron/db`'s
    // migrations, which routinely takes longer than Vitest's 5s default on a cold CI runner — the
    // identical reasoning packages/reporting/vitest.config.ts records for the identical cost.
    testTimeout: 30_000,
    hookTimeout: 60_000,
    coverage: {
      provider: "v8",
