import { makeT, registerCatalogue } from "@waitron/dashboard-kit";

const en = {
  "venue.route_duplicate":
    "A route already exists for this product or category in that service zone. Edit the existing route instead.",
  "venue.department_has_zones":
    "Move its active service zones to another department before deactivating this department.",
  "venue.status": "Status",
  "venue.active": "Active",
  "venue.inactive": "Inactive",
  "venue.edit": "Edit",
  "venue.delete": "Delete",
  "venue.save": "Save",
  "venue.cancel": "Cancel",
  "venue.confirm": "Confirm",
  "venue.confirm_remove": "Confirm removal",
  "venue.no_rows": "No entries yet.",
  "venue.products": "Products",
  "venue.zone": "Service zone",
  "venue.unconfigured": "Not configured",
  "venue.yes": "Yes",
  "venue.no": "No",
  "venue.display_order": "Display order",
  "venue.edit_department": "Edit department",
  "venue.edit_menu": "Edit menu",
  "venue.edit_offer": "Edit menu price",
  "venue.edit_hours": "Edit hours",
  "venue.edit_assignment": "Edit menu assignment",
  "venue.edit_route": "Edit route",
  "venue.price_invalid": "Enter a non-negative price with up to two decimal places.",
  "venue.order_invalid": "Enter a whole number of zero or more.",

  "nav.venue_operations": "Venue operations",
  "venue.title": "Venue operations",
  "venue.departments": "Departments",
  "venue.add_department": "Add department",
  "venue.name": "Department name",
  "venue.trading_name": "Trading name",
  "venue.service_style": "Service style",
  "venue.table_tab": "Table service",
  "venue.prepay": "Pay before preparation",
  "venue.invoice_first": "Pay then prepare",
  "venue.ticket_then_pay": "Prepare then pay",
  "venue.zones": "Service zones and menus",
  "venue.department": "Department",
  "venue.inherit": "Use department default",
  "venue.save_zone": "Save zone",
  "venue.make_available": "Make available",
  "venue.make_default": "Make default",
  "venue.default": "Default",
  "venue.hours": "Opening hours",
  "venue.menus": "Menus",
  "venue.add_menu": "Add menu",
  "venue.menu_name": "Menu name",
  "venue.product": "Product",
  "venue.price": "Price",
  "venue.price_was": "Was",
  "venue.price_inherited": "(product's own price)",
  "venue.variants": "Variants",
  "venue.offered": "Offered on this menu",
  "venue.offer_price_hint": "Leave the price empty to charge the product's own price.",
  "venue.variant_price_hint":
    "Leave a variant's price empty to charge its own price, or the product's if it has none. The hint shows the price that applies.",
  "venue.add_offer": "Add product to menu",
  "venue.save_price": "Save price",
  "venue.remove_offer": "Remove from menu",
  "venue.remove_offer_resets": "Its menu price and variant settings are cleared.",
  "venue.remove_offer_stays": "It stays on this menu through its section.",
  "venue.switch_offer_off": "Switch off on this menu",
  "venue.switch_offer_on": "Switch back on for this menu",
  "venue.offer_on": "On",
  "venue.offer_off": "Switched off",
  "venue.placement": "Where it sits",
  "venue.placement_top": "Top level",
  "venue.placement_section": "In a section",
  "venue.placement_both": "Top level and in a section",
  "venue.actions": "Actions",
  "venue.no_offers": "No products on this menu.",
  "venue.weekday": "Day",
  "venue.opens": "Opens",
  "venue.closes": "Closes",
  "venue.add_hours": "Add hours",
  "venue.routing": "Preparation routing",
  "venue.category": "Product category",
  "venue.product_or_category": "Product or category",
  "venue.all_zones": "All service zones",
  "venue.station": "Preparation station",
  "venue.no_preparation": "No preparation",
  "venue.add_route": "Add route",
  "venue.remove_route": "Remove route",
  "venue.required": "Fill in every required field.",
  "venue.form_error_heading": "There is a problem with this form",
  "venue.field_required": "This field is required.",
  "venue.time_distinct": "Opening and closing times must differ.",
  "venue.day.0": "Sunday",
  "venue.day.1": "Monday",
  "venue.day.2": "Tuesday",
  "venue.day.3": "Wednesday",
  "venue.day.4": "Thursday",
  "venue.day.5": "Friday",
  "venue.day.6": "Saturday",
  "venue.load_error": "The venue configuration could not be loaded.",
  "venue.save_error": "The change could not be saved.",
  "venue.empty": "No departments have been configured.",
  "venue.readiness": "Ready for service",
  "venue.readiness.ok": "Every active service zone is ready to take orders.",
  "venue.readiness.department_missing": "Create an active department before service.",
  "venue.readiness.zone_department_missing": "needs an active department.",
  "venue.readiness.zone_menu_missing": "needs a default menu.",
  "venue.readiness.menu_empty": "has no products for",
  "venue.readiness.route_missing": "needs a preparation route in",
  "venue.deactivate_department": "Deactivate department",
} as const;

const es: Record<keyof typeof en, string> = {
  "venue.route_duplicate":
    "Ya existe una ruta para este producto o categoría en esa zona de servicio. Edita la ruta existente.",
  "venue.department_has_zones":
    "Mueve sus zonas de servicio activas a otro departamento antes de desactivar este departamento.",
  "venue.status": "Estado",
  "venue.active": "Activo",
  "venue.inactive": "Inactivo",
  "venue.edit": "Editar",
  "venue.delete": "Eliminar",
  "venue.save": "Guardar",
  "venue.cancel": "Cancelar",
  "venue.confirm": "Confirmar",
  "venue.confirm_remove": "Confirmar eliminación",
  "venue.no_rows": "Todavía no hay entradas.",
  "venue.products": "Productos",
  "venue.zone": "Zona de servicio",
  "venue.unconfigured": "Sin configurar",
  "venue.yes": "Sí",
  "venue.no": "No",
  "venue.display_order": "Orden de visualización",
  "venue.edit_department": "Editar departamento",
  "venue.edit_menu": "Editar carta",
  "venue.edit_offer": "Editar precio de la carta",
  "venue.edit_hours": "Editar horario",
  "venue.edit_assignment": "Editar asignación de carta",
  "venue.edit_route": "Editar ruta",
  "venue.price_invalid": "Introduce un precio no negativo con hasta dos decimales.",
  "venue.order_invalid": "Introduce un número entero igual o mayor que cero.",

  "nav.venue_operations": "Operaciones del local",
  "venue.title": "Operaciones del local",
  "venue.departments": "Departamentos",
  "venue.add_department": "Añadir departamento",
  "venue.name": "Nombre del departamento",
  "venue.trading_name": "Nombre comercial",
  "venue.service_style": "Estilo de servicio",
  "venue.table_tab": "Servicio de mesa",
  "venue.prepay": "Pagar antes de preparar",
  "venue.invoice_first": "Pagar y después preparar",
  "venue.ticket_then_pay": "Preparar y después pagar",
  "venue.zones": "Zonas de servicio y cartas",
  "venue.department": "Departamento",
  "venue.inherit": "Usar el valor del departamento",
  "venue.save_zone": "Guardar zona",
  "venue.make_available": "Hacer disponible",
  "venue.make_default": "Establecer por defecto",
  "venue.default": "Por defecto",
  "venue.hours": "Horario de apertura",
  "venue.menus": "Cartas",
  "venue.add_menu": "Añadir carta",
  "venue.menu_name": "Nombre de la carta",
  "venue.product": "Producto",
  "venue.price": "Precio",
  "venue.price_was": "Antes",
  "venue.price_inherited": "(precio propio del producto)",
  "venue.variants": "Variantes",
  "venue.offered": "Se ofrece en esta carta",
  "venue.offer_price_hint": "Deja el precio vacío para cobrar el precio propio del producto.",
  "venue.variant_price_hint":
    "Deja vacío el precio de una variante para cobrar su propio precio o, si no tiene, el del producto. La sugerencia muestra el precio que se aplica.",
  "venue.add_offer": "Añadir producto a la carta",
  "venue.save_price": "Guardar precio",
  "venue.remove_offer": "Quitar de la carta",
  "venue.remove_offer_resets": "Se borran su precio y sus ajustes de variantes en esta carta.",
  "venue.remove_offer_stays": "Sigue en esta carta a través de su sección.",
  "venue.switch_offer_off": "Desactivar en esta carta",
  "venue.switch_offer_on": "Activar en esta carta",
  "venue.offer_on": "Activado",
  "venue.offer_off": "Desactivado",
  "venue.placement": "Dónde está",
  "venue.placement_top": "Nivel superior",
  "venue.placement_section": "En una sección",
  "venue.placement_both": "Nivel superior y en una sección",
  "venue.actions": "Acciones",
  "venue.no_offers": "Esta carta no tiene productos.",
  "venue.weekday": "Día",
  "venue.opens": "Abre",
  "venue.closes": "Cierra",
  "venue.add_hours": "Añadir horario",
  "venue.routing": "Rutas de preparación",
  "venue.category": "Categoría de producto",
  "venue.product_or_category": "Producto o categoría",
  "venue.all_zones": "Todas las zonas de servicio",
  "venue.station": "Estación de preparación",
  "venue.no_preparation": "Sin preparación",
  "venue.add_route": "Añadir ruta",
  "venue.remove_route": "Quitar ruta",
  "venue.required": "Completa todos los campos obligatorios.",
  "venue.form_error_heading": "Hay un problema con este formulario",
  "venue.field_required": "Este campo es obligatorio.",
  "venue.time_distinct": "Las horas de apertura y cierre deben ser distintas.",
  "venue.day.0": "Domingo",
  "venue.day.1": "Lunes",
  "venue.day.2": "Martes",
  "venue.day.3": "Miércoles",
  "venue.day.4": "Jueves",
  "venue.day.5": "Viernes",
  "venue.day.6": "Sábado",
  "venue.load_error": "No se pudo cargar la configuración del local.",
  "venue.save_error": "No se pudo guardar el cambio.",
  "venue.empty": "No hay departamentos configurados.",
  "venue.readiness": "Preparado para el servicio",
  "venue.readiness.ok": "Todas las zonas activas están preparadas para aceptar pedidos.",
  "venue.readiness.department_missing": "Crea un departamento activo antes del servicio.",
  "venue.readiness.zone_department_missing": "necesita un departamento activo.",
  "venue.readiness.zone_menu_missing": "necesita una carta predeterminada.",
  "venue.readiness.menu_empty": "no tiene productos para",
  "venue.readiness.route_missing": "necesita una ruta de preparación en",
  "venue.deactivate_department": "Desactivar departamento",
};

export const VENUE_SERVICE_STRINGS = { en, es };
registerCatalogue(VENUE_SERVICE_STRINGS);
export const t = makeT<keyof typeof en>();
