...
checkout, or typecheck fails on a module that exists. The main checkout goes stale in a way the
worktrees do not, because `worktree.py new` installs and nothing installs here. Observed symptom:
`tsc: command not found` in `packages/migrations` with
`WARN Local package.json exists, but node_modules missing` — on a `git push --delete` that carried
no TypeScript whatsoever, because the hook checks the working tree rather than the push.
