...
- **Multi-table writes share ONE transaction, and `withTenant` IS that transaction.** Write-path
  ...
  a default.** Queries on one transaction are awaited in turn, never `Promise.all`; no guard enforces
  it (receipt in [conventions-data.md](docs/developers/conventions-data.md)).
